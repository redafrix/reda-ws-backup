...
OUT_DIR="asynchvla_ws/stage8_ultimate/results/normal_libero_hard"
mkdir -p "$OUT_DIR"
...
for spec in "${TASKS[@]}"; do
  ...
  echo "[stage8] normal LIBERO hard baseline suite=$suite task=$task out=$out"
...
python3 asynchvla_ws/scripts/stage8_summarize_rollouts.py \
  --input-dir "$OUT_DIR" \
  --out-md asynchvla_ws/stage8_ultimate/reports/stage8_normal_libero_hard_task_baseline.md \
  --title "Stage 8 Normal LIBERO Hard-Task Baseline"
