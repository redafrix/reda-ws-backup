...
BOB_TASKS = [
    ...
    ("libero_spatial_with_milk", list(range(10))),
    ("libero_10_with_milk", list(range(10))),
    ("libero_spatial_with_yellow_book", list(range(10))),
    ("libero_object_with_yellow_book", list(range(10))),
    ("libero_goal_with_yellow_book", list(range(10))),
]
...
def rows_from_root(root: Path):
    for file in sorted(root.rglob("counterfactual_samples.jsonl")):
        try:
            with file.open() as f:
                for line in f:
                    if line.strip():
...
def summarize(root: Path) -> dict:
    ...
    for row in rows_from_root(root):
        ...
        terminal_ok = bool(outcome.get("terminal_success") or outcome.get("done_within_H") or outcome.get("terminal_done"))
...
def sync_from_bob(args, logs_dir: Path, errors: list[str]) -> None:
    if not args.bob_source:
        return
    dest = args.workspace / "asynchvla_ws/stage9_libero_pro_risk_data/data/final_20h/synced_from_bob"
    dest.mkdir(parents=True, exist_ok=True)
    log_path = logs_dir / "sync_from_bob.log"
    cmd = [
        "rsync",
        "-az",
        "-s",
        "--include=*/",
        "--include=counterfactual_samples.jsonl",
        "--include=summary.json",
        "--include=watchdog_summary.json",
        "--exclude=states/***",
        "--exclude=images/***",
        "--exclude=trace_frames/***",
        "--exclude=*",
        args.bob_source.rstrip("/") + "/",
        str(dest) + "/",
    ]
    with log_path.open("ab") as log:
        rc = subprocess.run(cmd, stdout=log, stderr=log).returncode
    if rc != 0:
        errors.append(f"{now()} sync_from_bob failed rc={rc}; see {log_path}")


def run_sync_train(args) -> None:
    stage_root = args.workspace / "asynchvla_ws/stage9_libero_pro_risk_data"
    run_root = stage_root / "data/final_20h/synced_from_bob"
    logs_dir = stage_root / "logs" / f"sync_train_{args.role}_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    append_report(report, f"## Sync/Training Watchdog Started: {args.role}\n\nStarted `{now()}` on `{socket.gethostname()}`.\n\nSync source: `{args.bob_source}`\n\nLocal root: `{run_root}`")
    while time.time() < stop_at:
        sync_from_bob(args, logs_dir, errors)
        write_dashboard(dashboard, args.role, "sync_train", run_root, started, errors)
        started_training = maybe_launch_training(args, run_root, report, logs_dir, started_training)
        time.sleep(600)
    write_dashboard(dashboard, args.role, "sync_train_finished", run_root, started, errors)
    final_summary = summarize(run_root)
    (run_root / "sync_train_summary.json").write_text(json.dumps(final_summary, indent=2, sort_keys=True) + "\n")
    append_report(report, f"## Sync/Training Watchdog Finished: {args.role}\n\nFinished `{now()}`.\n\n```json\n{json.dumps(final_summary, indent=2, sort_keys=True)}\n```")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--role", choices=["bob", "sam"], required=True)
    parser.add_argument("--mode", choices=["collect", "sync_train"], default="collect")
    ...
    parser.add_argument("--bob-source", default="")
    args = parser.parse_args()
    if args.mode == "sync_train":
        run_sync_train(args)
        return
