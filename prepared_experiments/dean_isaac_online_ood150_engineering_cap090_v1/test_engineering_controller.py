...
from online_isaac_runtime import (  # noqa: E402
    SelectionDecision,
    select_candidate_argmin_on_alarm,
)
...
class TestEngineeringController(unittest.TestCase):
    def test_01_main_below_alarm(self):
        ...
        decision = select_candidate_argmin_on_alarm(scores, main_threshold=A, selected_score_cap=C)
        self.assertEqual(decision.selected_candidate_index, 0)
        self.assertEqual(decision.selection_reason, "main_below_alarm_threshold")
        self.assertFalse(decision.is_intervention)

    def test_02_alarm_main_already_lowest(self):
        scores = [0.85, 0.90, 0.92, 0.88, 0.95, 0.99, 0.89, 0.91, 0.94]
        decision = select_candidate_argmin_on_alarm(scores, main_threshold=A, selected_score_cap=C)
        self.assertEqual(decision.selected_candidate_index, 0)
        self.assertEqual(decision.selection_reason, "main_is_lowest")
        self.assertFalse(decision.is_intervention)
    ...
    def test_03_best_alt_lower_than_main_but_above_cap(self):
        ...
        decision = select_candidate_argmin_on_alarm(scores, main_threshold=A, selected_score_cap=C)
        self.assertEqual(decision.selected_candidate_index, 0)
        self.assertEqual(decision.selection_reason, "best_alternative_above_cap")
        self.assertFalse(decision.is_intervention)
    ...
    def test_04_best_alt_lower_and_below_cap_selected(self):
        ...
        decision = select_candidate_argmin_on_alarm(scores, main_threshold=A, selected_score_cap=C)
        self.assertEqual(decision.selected_candidate_index, 3)
        self.assertEqual(decision.selection_reason, "argmin_on_alarm_cap_pass")
    ...
    def test_05_selected_index_range(self):
        for _ in range(50):
            ...
            decision = select_candidate_argmin_on_alarm(scores, main_threshold=A, selected_score_cap=C)
            self.assertIn(decision.selected_candidate_index, list(range(9)))
    ...
    def test_06_active_vs_shadow_execution_mapping(self):
        ...
        decision = select_candidate_argmin_on_alarm(scores, main_threshold=A, selected_score_cap=C)
        self.assertEqual(decision.selected_candidate_index, 4)

        # In ACTIVE mode: executed action is chunks_env[decision.selected_candidate_index]
        active_executed = chunks_env[decision.selected_candidate_index]
