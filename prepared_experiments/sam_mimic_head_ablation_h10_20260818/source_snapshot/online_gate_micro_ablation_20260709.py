...
DEAN_ROOT = Path("/media/rootalkhatib/My Passport/reda_ws/fiper_ws/cross_suite_official_ood_20260630")
COLLECTOR_DIR = Path("/media/rootalkhatib/My Passport/reda_ws/asynchvla_ws/src/data_collection_stage9")
            ...
            rows.append(
                {
                    ...
                    "initial_state_npz": raw.get("initial_state_npz", ""),
...
def compute_ace_metrics_logdet(ace_chunks_normalized: Any) -> np.ndarray:
...
def compute_ace_metrics_l2log(ace_chunks_normalized: Any) -> np.ndarray:
    arr = np.asarray(ace_chunks_normalized if ace_chunks_normalized is not None else [], dtype=np.float32)
...
def compute_ace_metrics(ace_chunks_normalized: Any, style: str = "logdet") -> np.ndarray:
    if style in {"l2log", "new_training"}:
        return compute_ace_metrics_l2log(ace_chunks_normalized)
    return compute_ace_metrics_logdet(ace_chunks_normalized)
...
def load_detector(name: str, model_dir: Path, device: torch.device) -> RiskDetector:
    if (model_dir / "metrics.json").exists():
        ...
        manifest = json.loads((model_dir / "PROMOTED_MODEL_MANIFEST.json").read_text())
        thresholds = manifest["thresholds"]
        selected_dims = [int(x) for x in manifest["feature_schema"].get("selected_uncertainty_dims", [])]
    ...
    for alias, source in [
        ...
        if alias not in thresholds and source in thresholds:
            thresholds[alias] = float(thresholds[source])
...
def restore_state_npz(env: Any, path: Path) -> dict[str, Any]:
    from sim_state_utils import set_state

    obs = env.reset()
    state = load_state_npz(path)
    try:
        set_state(env, state, hard_reset=False)
    except TypeError:
        set_state(env, state)
...
def resolve_optional_score_threshold(value: Any, detector: RiskDetector) -> float | None:
    if value is None:
        return None
    return resolve_score_threshold(value, detector)
...
def select_action(
    ...
    selection_rule: str = "strict_margin",
) -> tuple[int, str]:
    main = float(scores[0])
    ...
    if selection_rule == "argmin_on_alarm":
        if diff < min_margin:
            return 0, "insufficient_margin"
        if max_selected_score is not None and best > float(max_selected_score):
            return 0, "candidate_above_selected_score_cap"
        return best_idx, "argmin_on_alarm"
    if diff < min_margin:
        return 0, "insufficient_margin"
    if max_selected_score is not None and best > float(max_selected_score):
        return 0, "candidate_above_selected_score_cap"
...
def run_episode(
    ...
    try:
        if episode_spec:
            ...
            initial_state_npz = str(episode_spec.get("initial_state_npz") or "")
            if initial_state_npz:
                obs = restore_state_npz(env, Path(initial_state_npz))
            else:
                try:
                    init_states = torch.load(episode_spec["init_state_path"], map_location="cpu", weights_only=False)
                except TypeError:
                    init_states = torch.load(episode_spec["init_state_path"], map_location="cpu")
        ...
        while num_steps < max_steps and not success:
            ...
            ace = compute_ace_metrics(candidates.chunks_norm[1:], str(cfg.get("ace_metric_style", "logdet")))
            ...
            if policy != "simvla_only" and generate_alternatives:
                ...
                if timestep < min_timestep:
                    ...
                    proposed_idx, last_selection_reason = select_action(
                        ...
                        max_selected_score=resolve_optional_score_threshold(cfg.get("selection_max_selected_score"), detector),
                        selection_rule=str(cfg.get("selection_rule", "strict_margin")),
                    )
                if proposed_idx != 0:
