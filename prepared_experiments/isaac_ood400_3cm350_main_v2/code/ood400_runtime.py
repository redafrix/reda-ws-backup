...
def load_stats(path: Path) -> dict[str, dict[str, np.ndarray]]:
    ...
    if str(p).endswith(".npz"):
        ...
        return {
            "history": {
                "mean": np.asarray(raw["history_mean"], dtype=np.float32),
                "std": np.asarray(raw["history_std"], dtype=np.float32),
            },
            "action": {
                "mean": np.asarray(raw["action_mean"], dtype=np.float32),
                "std": np.asarray(raw["action_std"], dtype=np.float32),
            },
            "static": {
                "mean": np.asarray(raw["static_mean"], dtype=np.float32),
                "std": np.asarray(raw["static_std"], dtype=np.float32),
            },
        }
...
class OnlineRiskSelector:
    def __init__(
        ...
        *,
        ...
        main_threshold: float = 0.579133152961731,
        selected_score_cap: float = 0.90,
        main_threshold_name: str = "Best F1",
        selected_cap_name: str = "engineering_cap_0.90",
        min_delta: float = 0.0,
        device: str = "cuda:0",
    ) -> None:
        import torch
        ...
        self.main_threshold = float(main_threshold)
        self.selected_score_cap = float(selected_score_cap)
        ...
        self.thresholds_sha256 = (
            sha256_file(self.thresholds_path)
...
class OOD400EpisodeStore:
    ...
    def completed_episode_ids(self) -> set[str]:
        ...
        for line in self.summaries_path.read_text().splitlines():
            if line.strip():
                try:
                    d = json.loads(line)
                    completed.add(str(d["episode_id"]))
                except Exception:
                    pass
        return completed
    ...
    def save_episode(
        ...
        *,
        ...
        with (ep_dir / "decisions.jsonl").open("w", encoding="utf-8") as f:
