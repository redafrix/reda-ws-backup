...
class SmolVLMActionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if use_adaln:
            ...
            if self.predict_uncertainty:
                self.velocity_final_layer = FinalLayer(hidden_size, dim_action)
                self.logvar_final_layer = FinalLayer(hidden_size, dim_action)
            else:
    ...
    def _forward_concat(
        ...
        x_action = self.norm(x[:, :num_actions])
        if self.predict_uncertainty:
            v_t = self.velocity_head(x_action)
            logvar_t = self.logvar_head(x_action)
            return v_t, logvar_t
        return self.action_decoder(x_action)
    ...
    def _forward_adaln(
        ...
        if self.predict_uncertainty:
            v_t = self.velocity_final_layer(x, c)
            logvar_t = self.logvar_final_layer(x, c)
            return v_t, logvar_t
