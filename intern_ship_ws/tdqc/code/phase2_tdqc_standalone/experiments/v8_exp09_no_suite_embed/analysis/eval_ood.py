"""Full eval for v8_exp09_no_suite_embed (OOD Ablation)."""
...
ROOT     = Path(".")
EXP      = ROOT / "experiments/v8_exp09_no_suite_embed"
...
def run_eval():
    ...
    test_set    = TDQCDataset(str(ROOT / "data/v8_balanced/v8_unseen_obj_ood.pt"))
    ...
    fig.suptitle("TDQC v8 Exp09 — No Suite Embed (Ablation) (OOD test set)", fontsize=13, fontweight="bold")
    ...
    fig2.suptitle("TDQC v8 Exp09 — Per-step accuracy (Ablation)", fontsize=13)
    ax3 = axes2[0]
    for tau in THRESHOLDS:
    ...
    if history_file.exists():
        ...
        ax5.set_title("v8 Exp09 — Training curve (No Suite Embed)")
    ...
    # ── Summary ───────────────────────────────────────────────────────────────────
    print(f"\n{'='*55}")
    print(f"SUMMARY — v8_exp09_no_suite_embed (OOD)")
    print(f"{'='*55}")
