"""Full eval for v7_exp05_delta_task_large."""
...
ROOT     = Path(".")
EXP      = ROOT / "experiments/v7_exp05_delta_task_large"
...
def run_eval():
    ...
    test_set    = TDQCDataset(str(ROOT / "data/v7_22d/v7_22d_test.pt"))
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            q = model(x, batch["lengths"], task_ids=batch["task_id"])
    ...
    fig.suptitle("TDQC v7 Exp04 — Delta + Task Embed (Large)  (test set)", fontsize=13, fontweight="bold")
    ...
    fig2.suptitle("TDQC v7 Exp04 — Per-step accuracy at fixed thresholds", fontsize=13)
    ax3 = axes2[0]
    for tau in THRESHOLDS:
    ...
    if history_file.exists():
        ...
        ax5.set_title("v7 Exp04 — Training curve (Delta + Task Embed (Large))")
    ...
    # ── Summary ───────────────────────────────────────────────────────────────────
    print(f"\n{'='*55}")
    print(f"SUMMARY — v7_exp05_delta_task_large")
    print(f"{'='*55}")
    ...
    print(f"{'='*55}")
    print("\nAll plots saved to experiments/v7_exp05_delta_task_large/plots/")
