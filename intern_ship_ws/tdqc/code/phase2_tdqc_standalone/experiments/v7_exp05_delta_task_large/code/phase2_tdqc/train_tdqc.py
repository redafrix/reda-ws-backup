...
def parse_args():
    ...
    p.add_argument("--output_dir", type=str, default="experiments/v7_exp05_delta_task_large/runs/")
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in train_loader:
            ...
            q_online = model(x, batch["lengths"], task_ids=batch["task_id"])
            with torch.no_grad():
                q_target = target_model(x, batch["lengths"], task_ids=batch["task_id"])
