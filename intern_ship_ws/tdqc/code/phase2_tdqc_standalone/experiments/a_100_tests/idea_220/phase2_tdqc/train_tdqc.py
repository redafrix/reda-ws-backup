...
def main():
    ...
    print("Loading Dataset..."); dataset = TDQCDataset(args.dataset_path, max_horizon=150, is_train=True) # Truncation Mandate
    ...
    print("Concatenating features (this takes a few mins)... "); all_feats = torch.cat([ep.features for ep in train_set], dim=0)
    print("Computing normalization stats..."); stats = compute_feature_stats(all_feats)
    
    print("Starting training loop..."); mean, std = stats["mean"].to(device), stats["std"].to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            with torch.no_grad(): q_t1 = target_model(x, mask=batch["mask"]); q_t2 = target_model2(x, mask=batch["mask"]); q_target = torch.min(q_t1, q_t2)
            loss = tdqc_brier_td_loss(q_online, q_target, batch["failure"], batch["lengths"], batch["mask"])
            # Anti-Drift Penalty
            q_diff = torch.abs(q_online[:, 1:] - q_online[:, :-1])
            drift_loss = (q_diff * batch['mask'][:, 1:]).mean() * 0.01
