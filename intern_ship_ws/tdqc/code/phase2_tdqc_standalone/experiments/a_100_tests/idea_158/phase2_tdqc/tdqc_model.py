...
class TDQCTransformerCalibrator(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        v7 = torch.zeros_like(x); v7[:, 7:, :] = x[:, 7:, :] - x[:, :-7, :]
        v9 = torch.zeros_like(x); v9[:, 9:, :] = x[:, 9:, :] - x[:, :-9, :]
        logits = self.head(self.mlp(torch.cat([x, v1, v3, v5, v7, v9, torch.log1p(torch.abs(v1[:, :, 14:22])), torch.log1p(torch.abs(v9[:, :, 14:22]))], dim=-1))).squeeze(-1)
