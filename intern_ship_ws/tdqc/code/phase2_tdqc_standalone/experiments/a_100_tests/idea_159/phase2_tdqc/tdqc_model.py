...
class TDQCTransformerCalibrator(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        v6 = torch.zeros_like(x); v6[:, 6:, :] = x[:, 6:, :] - x[:, :-6, :]
        logits = self.head(self.mlp(torch.cat([x, v1, v2, v3, v4, v5, v6, torch.log1p(torch.abs(v1[:, :, 14:22])), torch.log1p(torch.abs(v6[:, :, 14:22]))], dim=-1))).squeeze(-1)
