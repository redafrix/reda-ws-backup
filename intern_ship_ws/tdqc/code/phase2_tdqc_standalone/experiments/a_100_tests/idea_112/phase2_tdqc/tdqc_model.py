...
class TDQCTransformerCalibrator(nn.Module):
    def __init__(
        ...
        self.convs = nn.ModuleList([CausalConv1d(hidden_dim, k) for k in [3, 5, 7]])
    ...
    def forward(
        ...
        x = torch.stack([c(x) for c in self.convs]).mean(dim=0)
        x = self.dropout(x)
        ...
        x = self.norm(x)
        logits = torch.stack([h(x) for h in self.head]).mean(dim=0).squeeze(-1)
