...
class TDQCTransformerCalibrator(nn.Module):
    ...
    def forward(
        ...
        if mask is not None:
            ...
            T = x.size(1)
        bias = torch.arange(T, device=x.device).view(1, -1) - torch.arange(T, device=x.device).view(-1, 1)
        bias = -torch.abs(bias).float() * 0.1
        attn_mask = (causal_mask + bias).unsqueeze(0).unsqueeze(0) + float_pad_mask.view(B, 1, 1, T)
            T = x.size(1)
        bias = torch.arange(T, device=x.device).view(1, -1) - torch.arange(T, device=x.device).view(-1, 1)
        bias = -torch.abs(bias).float() * 0.1
