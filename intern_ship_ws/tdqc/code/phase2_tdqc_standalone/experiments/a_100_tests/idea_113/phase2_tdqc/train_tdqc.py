...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            with torch.no_grad(): q_t1 = target_model(x, mask=batch["mask"]); q_t2 = target_model2(x, mask=batch["mask"]); stack = torch.stack([q_t1, q_t2]); q_target = stack.mean(dim=0) - 0.2 * stack.std(dim=0)
