...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            pred_x = model.input_proj(x[:, :-1, :]); target_x = model.input_proj(x[:, 1:, :])
            aux_loss = F.mse_loss(pred_x * batch["mask"][:, 1:, None], target_x * batch["mask"][:, 1:, None])
            loss = tdqc_brier_td_loss(q_online, q_target, batch["failure"], batch["lengths"], batch["mask"]) + 0.1 * aux_loss
