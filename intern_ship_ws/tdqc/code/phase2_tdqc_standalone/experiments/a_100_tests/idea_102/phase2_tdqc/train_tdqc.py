...
def evaluate(model, loader, mean, std, device):
    ...
    for batch in loader:
        ...
        raw_x = normalize_features(batch["features"], mean, std)
        delta_x = torch.zeros_like(raw_x)
        delta_x[:, 1:, :] = raw_x[:, 1:, :] - raw_x[:, :-1, :]
        x = torch.cat([raw_x, delta_x], dim=-1) # Input is now 44D
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        x = torch.cat([raw_x, delta_x], dim=-1) # Input is now 44D
            ...
            with torch.no_grad(): q_target = target_model(x, mask=batch["mask"])
