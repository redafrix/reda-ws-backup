...
class TDQCTransformerCalibrator(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        x_pad = F.pad(x.transpose(1, 2), (6, 0), mode='replicate')
        x_smooth = F.avg_pool1d(x_pad, kernel_size=7, stride=1).transpose(1, 2)
        v1 = torch.zeros_like(x_smooth); v1[:, 1:, :] = x_smooth[:, 1:, :] - x_smooth[:, :-1, :]
        v3 = torch.zeros_like(x_smooth); v3[:, 3:, :] = x_smooth[:, 3:, :] - x_smooth[:, :-3, :]
        v5 = torch.zeros_like(x_smooth); v5[:, 5:, :] = x_smooth[:, 5:, :] - x_smooth[:, :-5, :]
        # log-uncertainty deltas (cols 14-21)
        u_v1 = F.softplus(v1[:, :, 14:22])
        u_v5 = F.softplus(v5[:, :, 14:22])
        logits = self.head(self.mlp(torch.cat([x_smooth, v1, v3, v5, u_v1, u_v5], dim=-1))).squeeze(-1)
