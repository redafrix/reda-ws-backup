...
def infer(observation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Inference
        with torch.no_grad():
            if hasattr(model, "generate_actions_with_uncertainty") and getattr(model.config, "predict_uncertainty", False):
                outputs = model.generate_actions_with_uncertainty(
                ...
                outputs = model.generate_actions(
                    input_ids=lang['input_ids'],
                    image_input=images,
                    image_mask=image_mask,
                    proprio=proprio_tensor,
        ...
        # Check if output is a dict (uncertainty-aware) or just actions tensor
        if isinstance(outputs, dict):
            actions = outputs["action"].cpu().numpy()[0]
            path_variance = outputs["path_variance"].cpu().numpy()[0]
            last_step_variance = outputs["last_step_variance"].cpu().numpy()[0]
            logger.info(f"Uncertainty detected: path_var mean={path_variance.mean():.6f}")
            return {
                "actions": actions,
...
if __name__ == "__main__":
    if HAS_MSGPACK:
        logger.info("Using msgpack_numpy for efficient serialization")
    else:
        logger.warning("msgpack_numpy not installed, using JSON fallback")
    main()
