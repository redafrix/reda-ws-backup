...
def compute_ace_metrics(ace_chunks_normalized: Any, candidate_limit: int | None = None) -> np.ndarray:
...
class CompactRow:
    ...
    history4_by_k: dict[int, np.ndarray]
...
def input_fields_for_config(cfg: dict[str, Any]) -> list[str]:
    fields: list[str] = []
    if bool(cfg.get("include_action_tokens", True)):
        fields.append("main_candidate_action_chunk_normalized.sequence_tokens")
    if bool(cfg.get("include_action", True)):
        fields.append(f"main_candidate_action_chunk_normalized.{cfg.get('action_repr', 'flat')}")
    if bool(cfg.get("include_ace", False)):
        fields.append("ace_candidate_chunks_normalized.metrics")
    if bool(cfg.get("include_proprio", False)):
        fields.append("current.proprio")
    if bool(cfg.get("include_history", True)):
        history_fields = ["history.previous_proprio", "history.previous_executed_action"]
        if bool(cfg.get("include_ace_history", True)):
            history_fields.append("history.previous_ace_metrics")
        fields.extend(history_fields)
    if bool(cfg.get("dynamics_residual", False)):
        fields.append("past_transition_residual.proprio_t_minus_proprio_t_minus_1")
    if int(cfg.get("ace_candidate_limit", 0) or 0) == 4:
        fields.append("ace_candidate_chunks_normalized.first_4_candidates_only")
    if int(cfg.get("ace_temporal_stride", 1) or 1) > 1:
        fields.append(f"ace_features.sampled_every_{int(cfg.get('ace_temporal_stride'))}_timesteps")
    return fields


def select_ace_for_config(row: CompactRow, cfg: dict[str, Any]) -> np.ndarray:
    ace = row.ace4 if int(cfg.get("ace_candidate_limit", 0) or 0) == 4 else row.ace
    stride = int(cfg.get("ace_temporal_stride", 1) or 1)
    if stride > 1 and (row.timestep % stride) != 0:
        return np.zeros_like(ace)
    return ace


def select_history_for_config(row: CompactRow, cfg: dict[str, Any], history_steps: int) -> np.ndarray:
    history = row.history4_by_k[history_steps] if int(cfg.get("ace_candidate_limit", 0) or 0) == 4 else row.history_by_k[history_steps]
    include_ace_history = bool(cfg.get("include_ace_history", True))
    if not include_ace_history:
        return history[:, :15].astype(np.float32, copy=True)

    out = history.astype(np.float32, copy=True)
    stride = int(cfg.get("ace_temporal_stride", 1) or 1)
    if stride > 1:
        for pos in range(history_steps):
            t_hist = row.timestep - history_steps + pos
            if t_hist < 0 or (t_hist % stride) != 0:
                out[pos, 15:] = 0.0
    return out
...
def load_rows_from_refs(
            ...
            with source_path.open() as f:
                for line_no, line in enumerate(f, start=1):
                    ...
                    compact_row = CompactRow(
                        ...
                        ace4=compute_ace_metrics(row.get("ace_candidate_chunks_normalized"), candidate_limit=4),
    ...
    lookup: dict[tuple[str, int], tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]] = {}
    for r in all_loaded_rows:
        lookup[(r.episode_key, r.timestep)] = (r.proprio, r._raw_executed_action, r.ace, r.ace4)
    ...
    for r in all_loaded_rows:
        for k in history_steps_needed:
            ...
            history4_seq = np.zeros((k, 21), dtype=np.float32)
            for i in range(k):
                t_prev = r.timestep - k + i
                if t_prev >= 0:
                    prev_data = lookup.get((r.episode_key, t_prev))
                    if prev_data is not None:
                        prev_proprio, prev_act, prev_ace, prev_ace4 = prev_data
                        history_seq[i, :] = np.concatenate([prev_proprio, prev_act, prev_ace[:6]])
                        history4_seq[i, :] = np.concatenate([prev_proprio, prev_act, prev_ace4[:6]])
            r.history_by_k[k] = history_seq
            r.history4_by_k[k] = history4_seq
...
def vector_features(rows: list[CompactRow], cfg: dict[str, Any]) -> np.ndarray:
    ...
    for row in rows:
        ...
        if cfg.get("include_ace", False):
            vec_parts.append(select_ace_for_config(row, cfg))
        ...
        if cfg.get("include_history", False):
            vec_parts.append(select_history_for_config(row, cfg, history_steps).reshape(-1))
        if not vec_parts:
...
def sequence_features(rows: list[CompactRow], cfg: dict[str, Any]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    histories = np.stack([select_history_for_config(r, cfg, history_steps) for r in rows], axis=0).astype(np.float32)
    if bool(cfg.get("include_action_tokens", True)):
...
def make_seq_inputs(rows_by_split: dict[str, list[CompactRow]], cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    unique_group_names = sorted(list({group_name_for_row(r, cfg) for r in train_rows + val_rows}))
    if (bool(cfg.get("group_dro", False)) or bool(cfg.get("adversarial", False))) and len(unique_group_names) < 2:
        raise ValueError(
            f"group-aware job {cfg['name']} has only {len(unique_group_names)} group(s); "
            "check ref metadata or group_strategy"
        )
    group_to_id = {name: idx for idx, name in enumerate(unique_group_names)}
    group_ids_train = np.array([group_to_id[group_name_for_row(r, cfg)] for r in train_rows], dtype=np.int64)
    group_ids_val = np.array([group_to_id[group_name_for_row(r, cfg)] for r in val_rows], dtype=np.int64)
...
def train_supervised_model(model: nn.Module, train_ds: Dataset, val_ds: Dataset, cfg: dict[str, Any], device: torch.device) -> tuple[nn.Module, list[dict[str, Any]], int]:
    ...
    group_dro_eta = float(cfg.get("group_dro_eta", 1.0))
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for batch, yb in train_loader:
            ...
            if label_smoothing > 0.0:
                y_loss = yb * (1.0 - label_smoothing) + 0.5 * label_smoothing
            
            if survival_heads:
                bce_elements = nn.functional.binary_cross_entropy_with_logits(logits, y_loss, pos_weight=pos_weight, reduction="none")
                if loss_name == "focal":
                    prob = torch.sigmoid(logits.detach())
                    pt = torch.where(yb > 0.5, prob, 1.0 - prob)
                    bce_elements = ((1.0 - pt) ** float(cfg.get("focal_gamma", 2.0))) * bce_elements
                bce = bce_elements.mean(dim=-1)
            else:
                bce = nn.functional.binary_cross_entropy_with_logits(logits, y_loss, pos_weight=pos_weight, reduction="none")
                if loss_name == "focal":
                    prob = torch.sigmoid(logits.detach())
                    pt = torch.where(yb > 0.5, prob, 1.0 - prob)
                    bce = ((1.0 - pt) ** float(cfg.get("focal_gamma", 2.0))) * bce
            ...
            if group_dro:
                ...
                weights = nn.functional.softmax(group_losses * group_dro_eta, dim=0)
...
def evaluate_job(
    ...
    for split, rows in rows_by_split.items():
        ...
        ace_entropy = np.asarray([select_ace_for_config(r, cfg)[0] for r in rows], dtype=np.float32)
        ...
        for head_name in head_names:
            ...
            for q in ["q95", "q99"]:
                ...
                if res_norms is not None and "q95" in res_thresholds:
                    ...
                    res_alarm = np.zeros_like(h_scores, dtype=bool)
...
def run_one_job(
    ...
    if global_args.force and job_dir.exists():
    ...
    for split, rows in rows_by_split.items():
        ...
        group_ids = np.array([inputs["group_to_id"].get(group_name_for_row(r, cfg), 0) for r in rows], dtype=np.int64)
    ...
    static_dim = inputs["st_train"].shape[-1]
    train_group_counts = {
        name: int(np.sum(inputs["group_ids_train"] == idx))
        for name, idx in inputs["group_to_id"].items()
    }
    val_group_counts = {
        name: int(np.sum(inputs["group_ids_val"] == idx))
        for name, idx in inputs["group_to_id"].items()
    }
    train_ood_rows = [r for r in inputs["train_rows"] + inputs["val_rows"] if r.is_ood_split]
    ...
    audit_data = {
        ...
        "uses_group_metadata_as_training_label_only": bool(cfg.get("group_dro", False) or cfg.get("adversarial", False)),
        "uses_ood_rows_for_train": bool(train_ood_rows),
        "uses_future_timestep": False,
        "dynamics_delta_alignment": "past/current only: proprio_t - proprio_t_minus_1" if cfg.get("dynamics_residual", False) else "not_used",
        "hardstop_is_real_residual_gate": bool(cfg.get("dynamics_residual", False)),
        "history_dim": hist_dim,
        "current_feature_dim": static_dim,
        "input_fields": input_fields_for_config(cfg),
        "group_strategy": cfg.get("group_strategy", "ref_combined"),
        "num_train_groups": len(inputs["group_to_id"]),
        "train_group_counts": train_group_counts,
        "val_group_counts": val_group_counts,
    }
    if train_ood_rows:
        raise ValueError(f"Global Validation FAIL: {len(train_ood_rows)} OOD rows found in train/val rows")
    (job_dir / "FEATURE_AUDIT.json").write_text(json.dumps(audit_data, indent=2) + "\n")

    calib_scores = scores_by_split["success_calib_seen"]
    calib_ace = np.asarray([select_ace_for_config(r, cfg)[0] for r in rows_by_split["success_calib_seen"]], dtype=np.float32)
    ...
    metrics = evaluate_job(rows_by_split, scores_by_split, res_norms_by_split, thresholds, cfg)
    ...
    for split, rows in rows_by_split.items():
        ...
        for i, row in enumerate(rows):
            ...
            row_dict = {
                ...
                "ace_entropy": float(select_ace_for_config(row, cfg)[0]),
                "outcome": row.outcome,
...
def main() -> None:
    ...
    if args.only_job:
        ...
        if not jobs:
            raise ValueError(f"--only-job {args.only_job} did not match any configured job")
    if args.max_jobs:
    ...
    if args.force and (out_dir / "failed_jobs.jsonl").exists():
        (out_dir / "failed_jobs.jsonl").unlink()
