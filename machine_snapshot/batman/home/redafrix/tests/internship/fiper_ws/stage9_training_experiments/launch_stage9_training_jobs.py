...
from stage9_training_experiments.stage9_dataset import group_safe_split
...
def train_command(
    ...
    cmd = [
        ...
        "-u",
        "-m",
        "stage9_training_experiments.train_stage9_risk_model",
        "--split-dir",
        str(split_dir),
        "--output-dir",
        str(out_dir),
...
def queue_script(
    ...
    lines.append(
        ...
        + " ".join(q(x) for x in ["python3", "-u", "-m", "stage9_training_experiments.stage9_calibration", "--campaign-dir", str(campaign_dir / machine_role)])
    )
    ...
    run_long = [
        ...
        "-u",
        "-m",
        "stage9_training_experiments.launch_stage9_training_jobs",
        "--mode",
        "run_long_top",
        "--workspace-root",
        str(workspace_root),
...
def run_long_top(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for rank, (score, old_dir, config) in enumerate(top, 1):
        ...
        env = os.environ.copy()
        env["PYTHONPATH"] = str(Path.home() / ".local/lib/python3.10/site-packages") + ":" + str(Path(args.workspace_root) / "fiper_ws") + ":" + str(Path(args.workspace_root) / "asynchvla_ws/src") + ":" + env.get("PYTHONPATH", "")
    ...
    subprocess.run(
        ["python3", "-u", "-m", "stage9_training_experiments.stage9_calibration", "--campaign-dir", str(machine_dir)],
        cwd=args.workspace_root,
        env={**os.environ, "PYTHONPATH": str(Path.home() / ".local/lib/python3.10/site-packages") + ":" + str(Path(args.workspace_root) / "fiper_ws") + ":" + str(Path(args.workspace_root) / "asynchvla_ws/src") + ":" + os.environ.get("PYTHONPATH", "")},
        check=False,
    )
