...
def run_episode(
    ...
    try:
        ...
        while num_steps < max_steps and not success:
            ...
            selection_exhausted = policy != "simvla_only" and max_modifications > 0 and modification_count >= max_modifications
            ...
            if policy != "simvla_only" and generate_alternatives:
                ...
                if is_chunk_0:
                    selected_idx = 0
                    last_selection_reason = "skip_chunk_0"
                    score_list = [0.0] * n_samples
                    main_score = 0.0
                    selected_score = 0.0
                    action_l2 = np.zeros(n_samples, dtype=np.float32)
                else:
            ...
            # Execute FULL chunk
            sel_chunk = candidates.chunks_env[selected_idx]
            actions_executed = 0
            rew = 0.0
            done = False
            for i in range(len(sel_chunk)):
                proprio_np = obs_to_proprio(obs)
                action = sel_chunk[i].astype(np.float32)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Dean realtime SimVLA uncertainty policy v1 (chunk execution)")
...
def main() -> None:
    ...
    if not getattr(simvla_model.config, "predict_uncertainty", False) and args.policy != "simvla_only":
        raise RuntimeError("risk policies require a checkpoint with predict_uncertainty=True")
    ...
    if args.policy == "risk_base":
        base_dir = cfg.get("risk_model_base_dir")
        if not base_dir and "risk_model_job_dir" in cfg:
            fiper_ws = Path(__file__).resolve().parent.parent.parent
            base_dir = fiper_ws / cfg["risk_model_job_dir"]
        detector = load_detector("base", Path(base_dir), device)
    elif args.policy == "risk_unc_topk8":
        unc_dir = cfg.get("risk_model_unc_topk8_dir")
        if not unc_dir and "risk_model_job_dir" in cfg:
            fiper_ws = Path(__file__).resolve().parent.parent.parent
            unc_dir = fiper_ws / cfg["risk_model_job_dir"]
        detector = load_detector("unc_topk8", Path(unc_dir), device)
    ...
    write_json(
        out_dir / "run_manifest.json",
        {
            ...
            "checkpoint_sha256": sha256_file(checkpoint / "model.safetensors") if checkpoint.exists() else "",
            "risk_model_dir": str(detector.model_dir) if detector else "",
            ...
            "execute_policy": "chunk_execution_10_actions",
        },
    )
