...
POLICIES = ["simvla_only", "shadow_base", "risk_base", "shadow_topk8", "risk_topk8", "topk8_v2b_adaptive_horizon", "topk8_v2d_commit_gate"]
...
def run_episode(
    ...
    if policy not in {"topk8_v2b_adaptive_horizon", "topk8_v2d_commit_gate"} and execution_horizon not in {1, 10}:
    ...
    # V2B/V2D state variables
    adaptive_risk_trigger_count = 0
    horizon1_query_count = 0
    horizon10_query_count = 0
    last_chosen_horizon = 0
    risk_scores = []
    query_timesteps = []
    
    # V2D-specific metrics
    v2d_commit_count = 0
    v2d_replan_count = 0
    v2d_tails_committed = 0
    v2d_tails_discarded = 0
    v2d_active_chunk = None
    v2d_active_chunk_original_risk = None
    v2d_active_chunk_query_index = None
    v2d_active_chunk_start_timestep = None
    v2d_active_chunk_executed_count = 0
    
    start = time.time()
    env = None
    if policy == "topk8_v2b_adaptive_horizon":
        rows_path = out_dir / "step_scores_topk8_v2.jsonl"
    elif policy == "topk8_v2d_commit_gate":
        rows_path = out_dir / "step_scores_topk8_v2d.jsonl"
    else:
    ...
    try:
        ...
        while num_steps < max_steps and not success:
            timestep = num_steps
            chosen_horizon = execution_horizon
            ...
            if policy == "topk8_v2b_adaptive_horizon":
                ...
                # Score only the main chunk
                score_arr = score_candidates(detector, hist, candidates.chunks_norm[:1], ace, proprio_np, candidates.features_49d[:1], device)
                main_risk = float(score_arr[0])
                risk_scores.append(main_risk)
                scores_seen.append(main_risk)
                selected_scores.append(main_risk)
                q95 = detector.thresholds["q95"]
                risk_triggered = main_risk >= q95
                if risk_triggered:
                    adaptive_risk_trigger_count += 1
                    chosen_horizon = 1
                    horizon1_query_count += 1
                else:
                    chosen_horizon = 10
                    horizon10_query_count += 1
                selected_score = main_risk
                main_score = main_risk
                last_selection_reason = "horizon_1" if risk_triggered else "horizon_10"
                # Track history variables
                previous_execution_horizon = last_chosen_horizon
                last_chosen_horizon = chosen_horizon
                query_timesteps.append(timestep)
                history_valid_count = len(history)
                history_padding_count = int(cfg.get("history_steps", 16)) - history_valid_count
                if len(query_timesteps) > 1:
                    history_time_stride_summary = [query_timesteps[i] - query_timesteps[i-1] for i in range(1, len(query_timesteps))]
                else:
                    history_time_stride_summary = []
            elif policy == "topk8_v2d_commit_gate":
                assert detector is not None
                score_arr = score_candidates(detector, hist, candidates.chunks_norm[:1], ace, proprio_np, candidates.features_49d[:1], device)
                fresh_risk = float(score_arr[0])
                q95 = detector.thresholds["q95"]
                
                # Check if we have an active chunk tail to evaluate
                if v2d_active_chunk is not None and v2d_active_chunk_executed_count == 5:
                    if fresh_risk < q95:
                        phase = "tail5_committed"
                        commit_decision = "commit"
                        old_tail_used = True
                        replan_triggered = False
                        chosen_horizon_val = 5
                        selected_chunk = v2d_active_chunk
                        
                        v2d_commit_count += 1
                        v2d_tails_committed += 1
                    else:
                        phase = "replanned_first5"
                        commit_decision = "replan"
                        old_tail_used = False
                        replan_triggered = True
                        chosen_horizon_val = 5
                        
                        v2d_active_chunk = candidates.chunks_env[0].copy()
                        v2d_active_chunk_original_risk = fresh_risk
                        v2d_active_chunk_query_index = query_index
                        v2d_active_chunk_start_timestep = timestep
                        v2d_active_chunk_executed_count = 5
                        
                        selected_chunk = v2d_active_chunk
                        
                        v2d_replan_count += 1
                        v2d_tails_discarded += 1
                else:
                    phase = "first5"
                ...
                risk_scores.append(fresh_risk)
                scores_seen.append(fresh_risk)
                selected_scores.append(fresh_risk)
                
                if phase == "first5":
                    action_source = "fresh_chunk"
                elif phase == "replanned_first5":
                    action_source = "replanned_chunk"
                else:
                    action_source = "original_tail"
            elif policy != "simvla_only" and generate_alternatives:
                assert detector is not None
            ...
            if policy != "topk8_v2d_commit_gate":
            ...
            if policy == "topk8_v2b_adaptive_horizon":
                chosen_horizon_val = chosen_horizon
            elif policy != "topk8_v2d_commit_gate":
                chosen_horizon_val = execution_horizon
            
            if policy == "topk8_v2d_commit_gate" and phase == "tail5_committed":
                offset = 5
            else:
                offset = 0
            
            horizon_limit = len(selected_chunk) - offset
            steps_to_run = min(chosen_horizon_val, horizon_limit)
            
            for action_index in range(steps_to_run):
                chunk_action_index = offset + action_index
                pre_proprio = obs_to_proprio(obs)
                action = selected_chunk[chunk_action_index].astype(np.float32)
            ...
            if policy == "topk8_v2b_adaptive_horizon":
                append_jsonl(
                    rows_path,
                    {
                        "schema_version": "topk8_v2b_feature_preserving_adaptive_horizon_step_v1",
                        "policy": policy,
                        "suite": suite,
                        "task_id": task_id,
                        ...
                        "previous_execution_horizon": previous_execution_horizon,
                        "history_valid_count": history_valid_count,
                        "history_padding_count": history_padding_count,
                        "history_time_stride_summary": history_time_stride_summary,
                        "main_seed": int(seeds[0]),
                        "main_risk": main_risk,
                        "threshold_used": q95,
                        "chosen_execution_horizon": chosen_horizon,
                ...
                append_jsonl(
                    rows_path,
                    {
                        "schema_version": "topk8_v2d_commit_gate_step_v1",
                        "policy": policy,
                        "suite": suite,
                        "task_id": task_id,
                        "episode_index": episode_index,
                        ...
                        "old_tail_used": old_tail_used,
                        "replan_triggered": replan_triggered,
                        "history_valid_count": history_valid_count,
                        "ace_features_present": True,
    ...
    if policy == "topk8_v2b_adaptive_horizon":
        ep_sum.update({
            "adaptive_risk_trigger_count": int(adaptive_risk_trigger_count),
            "horizon1_query_count": int(horizon1_query_count),
            "horizon10_query_count": int(horizon10_query_count),
            "risk_score_mean": float(np.mean(risk_scores)) if risk_scores else 0.0,
            "risk_score_max": float(np.max(risk_scores)) if risk_scores else 0.0,
            "threshold_used": float(detector.thresholds["q95"]) if detector else 0.0,
            "execution_horizon_policy": "adaptive_10_or_1"
        })
    elif policy == "topk8_v2d_commit_gate":
        ep_sum.update({
            "v2d_commit_count": int(v2d_commit_count),
            "v2d_replan_count": int(v2d_replan_count),
            "v2d_tails_committed": int(v2d_tails_committed),
            "v2d_tails_discarded": int(v2d_tails_discarded),
            "risk_score_mean": float(np.mean(risk_scores)) if risk_scores else 0.0,
            "risk_score_max": float(np.max(risk_scores)) if risk_scores else 0.0,
            "threshold_used": float(detector.thresholds["q95"]) if detector else 0.0,
            "execution_horizon_policy": "commit_gate"
        })
    return ep_sum
...
def main() -> None:
    ...
    elif args.policy in {"shadow_topk8", "risk_topk8", "topk8_v2b_adaptive_horizon", "topk8_v2d_commit_gate"}:
