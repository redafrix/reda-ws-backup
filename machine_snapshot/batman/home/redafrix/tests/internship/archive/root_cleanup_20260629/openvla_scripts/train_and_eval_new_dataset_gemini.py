...
# Directories
OUTPUT_DIR = "/media/rootalkhatib/My Passport/reda_ws/openvla_oft_ws_20260616/offline_risk_experiments/openvla_new1891_risk_20260618"
DATASET_DIR = "/media/rootalkhatib/My Passport/reda_ws/openvla_oft_ws_20260616/outputs/openvla_goal_object_pro_risk_data_10000ep_round_robin_20260617"
...
def evaluate_on_test(model, stats, test_rows, test_ids, ep_map, max_fail_steps=800):
    model.eval()
    ...
    with torch.no_grad():
        for batch, _ in test_loader:
            ...
            logits = model(batch)
            probs = torch.sigmoid(logits)
            preds_test.extend(probs.cpu().numpy())
    preds_test = np.array(preds_test)
    
    # Find thresholds on validation split
    best_f1 = -1.0
    best_thresh = 0.5
    for th in np.arange(0.01, 1.00, 0.01):
        m = compute_metrics(y_test, preds_test, threshold=th)
        if m["f1"] > best_f1:
            best_f1 = m["f1"]
            best_thresh = th
            
    q90_th = np.percentile(preds_test[y_test == 0.0], 90)
    q95_th = np.percentile(preds_test[y_test == 0.0], 95)
...
def main():
    ...
    metrics_800, early_800, n_fail_800, fa_800, n_succ_800 = evaluate_on_test(
        model_800, stats_800, test_800, test_ids, ep_map_800, max_fail_steps=800
    )
    ...
    metrics_300, early_300, n_fail_300, fa_300, n_succ_300 = evaluate_on_test(
        model_300, stats_300, test_300, test_ids, ep_map_300, max_fail_steps=300
    )
    ...
    report_path = os.path.join(OUTPUT_DIR, "reports", "NEW_DATASET_1891EP_EVALUATION_REPORT_20260618.md")
    os.makedirs(os.path.dirname(report_path), exist_ok=True)
    
    # Construct Markdown Report Content
    report_content = f"""# Comparative Evaluation Report: New 1,891-Episode Dataset
