...
POLICIES = ["simvla_only", "shadow_base", "risk_base", "shadow_topk8", "risk_topk8", "topk8_v2c_h5_adaptive_horizon"]
...
def run_episode(
    ...
    if policy != "topk8_v2c_h5_adaptive_horizon" and execution_horizon not in {1, 10}:
    ...
    start = time.time()
    env = None
    if policy == "topk8_v2c_h5_adaptive_horizon":
        rows_path = out_dir / "step_scores_topk8_v2c.jsonl"
    else:
        ...
        while num_steps < max_steps and not success:
            ...
            if policy == "topk8_v2c_h5_adaptive_horizon":
                ...
                if risk_triggered:
                    ...
                    chosen_horizon = 5
                    horizon5_query_count += 1
                else:
                    chosen_horizon = 10
                    horizon10_query_count += 1
                selected_score = main_risk
                main_score = main_risk
                last_selection_reason = "horizon_5" if risk_triggered else "horizon_10"
            ...
            chosen_horizon_val = chosen_horizon if policy == "topk8_v2c_h5_adaptive_horizon" else execution_horizon
            for action_index in range(min(chosen_horizon_val, len(selected_chunk))):
            ...
            if policy == "topk8_v2c_h5_adaptive_horizon":
                append_jsonl(
                    rows_path,
                    {
                        "schema_version": "topk8_v2c_h5_adaptive_horizon_step_v1",
                        "policy": policy,
                        "suite": suite,
                        "task_id": task_id,
    ...
    if policy == "topk8_v2c_h5_adaptive_horizon":
        ep_sum.update({
            ...
            "horizon5_query_count": int(horizon5_query_count),
            ...
            "execution_horizon_policy": "adaptive_10_or_5"
        })
    return ep_sum
...
def main() -> None:
    ...
    elif args.policy in {"shadow_topk8", "risk_topk8", "topk8_v2c_h5_adaptive_horizon"}:
