...
OUT_DIR = EXP_DIR / "threshold_sweep_ood180_20260629"
SEEDS = [0, 1, 2, 42, 43]
...
def main():
    ...
    for method, spec in method_specs.items():
        for policy_kind, thresholds in [("any", any_thresholds), ("mass_above_1", mass_thresholds)]:
            ...
            for seed in SEEDS:
                ...
                ep_scores = [scores for scores, is_ood in zip(ep_scores_all, ood_mask) if is_ood]
    ...
    csv_path = OUT_DIR / "official_fiper_ood180_threshold_sweep.csv"
    df.to_csv(csv_path, index=False)

    lines = []
    lines.append("# Official FIPER OOD180 Threshold Sweep")
    lines.append("")
    lines.append("- No retrain.")
    lines.append("- Reuses official FIPER RND checkpoints already trained on in-domain data.")
    lines.append("- OOD test set only: 180 episodes = 149 success + 31 failure.")
    lines.append("- Scores are official FIPER `tvt_quantile` q=0.95 normalized scores; official deployment point is `any_1`.")
    ...
    report_path = OUT_DIR / "OFFICIAL_FIPER_OOD180_THRESHOLD_SWEEP_20260629.md"
