...
# Directories
OUTPUT_DIR = "/media/rootalkhatib/My Passport/reda_ws/openvla_oft_ws_20260616/offline_risk_experiments/openvla_new1894_risk_corrected_20260618"
...
def main():
    ...
    report_path = os.path.join(OUTPUT_DIR, "reports", "NEW_DATASET_1894EP_CORRECTED_EVALUATION_REPORT_20260618.md")
    os.makedirs(os.path.dirname(report_path), exist_ok=True)
    
    # Construct Markdown Report Content
    report_content = f"""# Corrected Comparative Evaluation Report: New {total_eps}-Episode Dataset

This corrected report compares the performance of the `SeqRiskModel` temporal Transformer risk model trained on the newly collected {total_eps}-episode `libero_goal_object` dataset on Bob under two failure logging horizons: **800 steps max** (default) vs. **300 steps max** (truncated).
