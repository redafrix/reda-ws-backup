...
def overlay_frame(frame, lines):
    ...
    draw.rectangle((0, 0, img.width, box_h), fill=(0, 0, 0, 175))
    for i, line in enumerate(lines):
        draw.text((pad, pad + i * line_h), line, fill=(255, 255, 255, 255), font=font if i == 0 else small)
...
def run_episode(policy_name, task_id, reset_seed, rollout_idx, task_suite, policy, risk_model, stats, thresholds, args, paths, device):
    ...
    episode_id = f"pi05_{args.suite}_t{task_id}_s{reset_seed}_{policy_name}"
    ...
    overlay_lines = [
        f"{args.suite} | task {task_id} | seed {reset_seed} | init {init_state_idx if 'init_state_idx' in locals() else 'NA'}",
        f"{task_description}",
        f"OUTCOME: {outcome_str} | steps={final_len} | policy={policy_name}",
    ]
    replay_images = [overlay_frame(x, overlay_lines) for x in replay_images]
    imageio.mimwrite(
        video_path,
    ...
    summary = {
        ...
        "task_id": task_id,
        "reset_seed": reset_seed,
        "init_state_idx": int(init_state_idx) if 'init_state_idx' in locals() else None,
        "success": success,
        "outcome": outcome_str,
        "steps": final_len,
...
def main():
    ...
    parser.add_argument("--suite", type=str, required=True, choices=["libero_goal_swap", "libero_goal_task", "libero_goal_object", "libero_goal_object_ood"])
    parser.add_argument("--seed-start", type=int, default=200)
    parser.add_argument("--seed-list", type=str, default="", help="Comma-separated reset seeds. Overrides --seed-start/--episodes-per-task when provided.")
    parser.add_argument("--seed-list-file", type=str, default="", help="JSON file containing a list of reset seeds. Overrides --seed-list when provided.")
    ...
    print(f"Initializing task suite {args.suite}...", flush=True)
    ...
    if args.seed_list_file:
        with open(args.seed_list_file) as f:
            seeds = [int(x) for x in json.load(f)]
    elif args.seed_list.strip():
        seeds = [int(x.strip()) for x in args.seed_list.split(",") if x.strip()]
    else:
        seeds = [args.seed_start + ep for ep in range(args.episodes_per_task)]
    if len(seeds) != len(set(seeds)):
        raise ValueError("Seed list contains duplicates; this breaks resume keys and paired comparison.")
    args.episodes_per_task = len(seeds)
    ...
    manifest = {
        ...
        "seed_list": seeds,
        "init_state_policy": "init_state_idx = episode_index % len(initial_states); with 100 episodes and 50 LIBERO init states, each init state is used exactly twice.",
        "max_steps": args.max_steps,
