...
def _get_object_state(
    ...
    if moving_speed is None:
        object_state = _get_static_object_state(
            object_range_bbox, object_z, random_orientation, friction
        )
    else:
...
def _get_static_object_state(object_range_bbox, object_z, random_orientation, friction=0.0):
    ...
    # STATIC_STABLE_SPAWN_V2_PATCH:
    # For static manipulation data, avoid arbitrary 3D rotations that make
    # bottles/cans/cups spawn sideways and roll. Keep yaw variation only.
    if random_orientation == "yaw_only":
        yaw = random.uniform(-np.pi, np.pi)
        object_quat = np.array([np.cos(yaw / 2.0), 0.0, 0.0, np.sin(yaw / 2.0)])
    else:
    ...
    return {"pos": object_position, "quat": object_quat, "friction": friction}
...
def main(args):
    ...
    # STATIC_STABLE_SPAWN_V2_PATCH:
    # Stable static manipulation mode:
    # - imply static_objects
    # - remove intentional initial object velocity
    # - request stable/upright spawn behavior downstream
    if getattr(args, "static_stable_spawn", False):
        args.static_objects = True
        sim_cfg.setdefault("scene", {}).setdefault("objects", {})["static_stable_spawn"] = True
        sim_cfg.setdefault("scene", {}).setdefault("objects", {})["random_orientation"] = "yaw_only"
        orig_friction = sim_cfg.setdefault("scene", {}).setdefault("objects", {}).get("friction", [0.5, 1.5])
        if isinstance(orig_friction, list):
            f_val = max(orig_friction)
        else:
            f_val = float(orig_friction)
        sim_cfg["scene"]["objects"]["friction"] = [max(f_val, 3.0), max(f_val, 3.0)]
        sim_cfg.setdefault("scene", {}).setdefault("objects", {})["angular_damping"] = max(
            float(sim_cfg.setdefault("scene", {}).setdefault("objects", {}).get("angular_damping", 0.05)),
            2.0,
        )
        logging.info("STATIC_STABLE_SPAWN_V2 enabled: static_objects=True, yaw-only/upright orientation, friction>=3.0, angular_damping>=2.0.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Static V2: prefer upright/stable spawn orientation and higher friction/damping for static manipulation data.",
    )
