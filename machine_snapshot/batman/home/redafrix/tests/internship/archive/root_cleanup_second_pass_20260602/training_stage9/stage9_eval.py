...
def binary_metrics_from_arrays(y: np.ndarray, p: np.ndarray) -> dict[str, Any]:
    return {
        "n": int(len(y)),
...
def binary_metrics(labels: list[str], probs: list[float], include_uncertain_as_safe: bool = False) -> dict[str, Any]:
    ...
    return binary_metrics_from_arrays(y, p)
...
def continuous_regression_metrics(items: list[dict[str, Any]]) -> dict[str, Any]:
    preds = np.array([float(x["risk_prob"]) for x in items])
    targets = np.array([float(x["target_value"]) for x in items])
    weights = np.array([float(x["weight"]) for x in items])
    mae = float(np.mean(np.abs(preds - targets)))
    mse = float(np.mean((preds - targets) ** 2))
    rmse = float(math.sqrt(mse))
    w_mae = float(np.sum(np.abs(preds - targets) * weights) / max(1e-9, np.sum(weights)))
    w_mse = float(np.sum(((preds - targets) ** 2) * weights) / max(1e-9, np.sum(weights)))
    pearson = None
    spearman = None
    ...
    for x in items:
        ...
        lbl = x["label"]
        prob = x["risk_prob"]
        is_high = (tgt >= 0.75 or lbl == "VALIDATED_BAD")
        is_low = (tgt <= 0.20 or lbl == "GOOD_STRONG")
        y_bin_all.append(1 if is_high else 0)
        p_bin_all.append(prob)
        if is_high:
            y_bin_clean.append(1)
            p_bin_clean.append(prob)
        elif is_low:
            y_bin_clean.append(0)
            p_bin_clean.append(prob)
    clean_bin = binary_metrics_from_arrays(np.array(y_bin_clean), np.array(p_bin_clean)) if y_bin_clean else {}
    all_non_bad = binary_metrics_from_arrays(np.array(y_bin_all), np.array(p_bin_all)) if y_bin_all else {}
    return {
        "mae": mae,
        "mse": mse,
        "rmse": rmse,
        ...
        "pearson": pearson,
        "spearman": spearman,
        "clean_binary": clean_bin,
        "bad_vs_all_non_bad": all_non_bad,
    }


def summarize_predictions(rows: list[dict[str, Any]], target_mode: str = "") -> dict[str, Any]:
    ...
    for split, items in by_split.items():
        ...
        if target_mode == "continuous_regression":
            split_summary["continuous_regression"] = continuous_regression_metrics(items)
            split_summary["clean_binary"] = split_summary["continuous_regression"]["clean_binary"]
            split_summary["bad_vs_all_non_bad"] = split_summary["continuous_regression"]["bad_vs_all_non_bad"]
...
def write_eval_report(path: Path, config: dict[str, Any], metrics: dict[str, Any]) -> None:
    ...
    for split, values in metrics.get("splits", {}).items():
        ...
        if "continuous_regression" in values:
            reg = values["continuous_regression"]
            lines.extend([
                f"- MAE: `{reg.get('mae')}`",
                f"- MSE: `{reg.get('mse')}`",
                f"- RMSE: `{reg.get('rmse')}`",
                f"- Weighted MAE: `{reg.get('weighted_mae')}`",
                f"- Weighted MSE: `{reg.get('weighted_mse')}`",
                f"- Pearson Correlation: `{reg.get('pearson')}`",
                f"- Spearman Correlation: `{reg.get('spearman')}`",
            ])
        lines.extend([
