...
def train(debug=True, split_name=None):
    ...
    (CK / 'predictions.json').write_text(json.dumps(pred_dump, cls=NpEncoder))
    
    report_name = 'stage5_debug_rater_metrics.md' if debug else f'stage5_{split_name}_metrics.md'
    lines = ['# Stage 5 Rater Metrics: ' + split_id, '', f'- Checkpoint dir: `{CK}`', '', '## Metrics JSON', '', '```json', json.dumps(out, indent=2, cls=NpEncoder), '```']
    RROOT.mkdir(parents=True, exist_ok=True)
    (RROOT / report_name).write_text('\n'.join(lines) + '\n')
    print('trained', split_id, json.dumps(out['parts'], cls=NpEncoder), RROOT / report_name)
    return out
