...
def split_dataset(input_path, output_dir, train_ratio=0.8, val_ratio=0.1):
    ...
    def balance_per_suite(eps_dict, label):
        ...
        for suite, eps in eps_dict.items():
            ...
            if len(succs) == 0 or len(fails) == 0:
                print(f"  [DROPPED] {label} [{suite}]: Has {len(succs)} successes and {len(fails)} failures. Dropping completely to prevent bias.")
                continue
                
            m = min(len(succs), len(fails))
            print(f"  [KEPT]    {label} [{suite}]: {len(succs)} succ, {len(fails)} fail -> balancing to {m} of each.")
...
if __name__ == "__main__":
    ...
    output_directory = "/media/rootalkhatib/My Passport/reda_ws/intern_ship_ws/tdqc/code/phase2_tdqc_standalone/experiments/v9_exp03/data"
