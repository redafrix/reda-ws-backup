...
def run_comprehensive_audit():
    ...
    if os.path.exists(TRAINING_SPLIT_PATH):
        ...
        for bucket_name, ep_uids in buckets.items():
            # Only consider training/validation/calibration buckets as "training/model contamination source"
    ...
    def audit_run_root(root_dir, expected_seeds_range):
        run_data = {}
        ...
        for cf in sorted(config_files):
            try:
                ...
                if "risk" in policy:
                    ...
                    if main_thresh != 0.3 or streak_thresh != 0.3:
                        config_errors.append(f"{os.path.basename(cf)}: Risk thresholds are not 0.3 (main={main_thresh}, streak={streak_thresh})")
        ...
        for t in tasks:
            ...
            for pol in ["original_simvla", "modified_simvla", "modified_h10_risk_topk8"]:
                ...
                # Check supervisor log for tasks
                ...
                # Horizon and Success Semantics check
                for ep in episode_summaries:
                    ...
                    # Failed episode check (must be exactly 300 steps or divisible by 10)
            ...
            # Check Seed Parity across policies for the same task
            if len(task_policies) == 3:
                s_orig = [ep.get("reset_seed") for ep in task_policies["original_simvla"]]
                s_mod = [ep.get("reset_seed") for ep in task_policies["modified_simvla"]]
                s_risk = [ep.get("reset_seed") for ep in task_policies["modified_h10_risk_topk8"]]
                
                # Check counts first
                min_len = min(len(s_orig), len(s_mod), len(s_risk))
                if not (s_orig[:min_len] == s_mod[:min_len] == s_risk[:min_len]):
                    seed_parity_errors.append(f"Task {task_num} seed mismatch at indices 0-{min_len-1}")
                if len(set(s_orig)) != len(s_orig):
                    seed_parity_errors.append(f"Task {task_num} original policy has duplicate reset seeds")
                    
                # Training contamination/leakage check for evaluation seeds
            ...
            # Paired analysis if complete
            if len(task_policies) == 3 and all(len(eps) >= min_len for eps in task_policies.values() if eps):
                # Align seed outcomes
                align_orig = {ep.get("reset_seed"): ep.get("success", False) or ep.get("outcome") == "success" for ep in task_policies["original_simvla"]}
                align_mod = {ep.get("reset_seed"): ep.get("success", False) or ep.get("outcome") == "success" for ep in task_policies["modified_simvla"]}
                align_risk = {ep.get("reset_seed"): ep.get("success", False) or ep.get("outcome") == "success" for ep in task_policies["modified_h10_risk_topk8"]}
                
                rescues = 0
                regressions = 0
                for seed in align_orig:
                    if seed in align_mod and seed in align_risk:
                        o_succ = align_orig[seed]
                        m_succ = align_mod[seed]
                        r_succ = align_risk[seed]
                        # risk aware vs modified simvla
                        if r_succ and not m_succ:
                            rescues += 1
                        elif not r_succ and m_succ:
                            regressions += 1
                task_data[task_num]["paired_risk_vs_mod"] = {
                    "rescues": rescues,
        ...
        # Check supervisor log for OOMs or KeyboardInterrupts
        sup_log = os.path.join(root_dir, "sweep_supervisor.log")
        run_data["log_issues"] = check_log_errors(sup_log)
        
        return run_data

    # 10ep run uses seeds [0, 10)
    audit_data["10ep_run"] = audit_dir_info = audit_run_root(ROOT_10EP, set(range(10)))
    
    # 100ep run uses seeds [10, 110)
    audit_data["100ep_run"] = audit_run_root(ROOT_100EP, set(range(10, 110)))
