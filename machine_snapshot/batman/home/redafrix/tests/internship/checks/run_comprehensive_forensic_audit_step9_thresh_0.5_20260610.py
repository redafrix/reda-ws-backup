...
ROOT_100EP = "/media/rootalkhatib/My Passport/reda_ws/fiper_ws/trash/h10_goal_object_ood_all_tasks_100ep_threshold_0.5_20260610"
...
def run_comprehensive_audit():
    ...
    def audit_run_root(root_dir, expected_seeds_range, target_thresh=None):
        ...
        for cf in sorted(config_files):
            try:
                ...
                if "risk" in policy and target_thresh is not None:
                    ...
                    if main_thresh != target_thresh or streak_thresh != target_thresh:
        ...
        # In ROOT_100EP threshold 0.5, we only ran "modified_h10_risk_topk8"
        # original_simvla and modified_simvla are checked in ROOT_10EP or ROOT_100EP (0.3)
        policies_to_check = ["modified_h10_risk_topk8"] if "threshold_0.5" in root_dir else ["original_simvla", "modified_simvla", "modified_h10_risk_topk8"]
        ...
        for t in tasks:
            ...
            # Check training contamination/leakage check for evaluation seeds
    ...
    audit_data["100ep_run"] = audit_run_root(ROOT_100EP, set(range(10, 110)), target_thresh=0.5)
