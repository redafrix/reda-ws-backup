...
NEW_ROOT = Path("/media/rootalkhatib/My Passport/reda_ws/fiper_ws/trash/h10_goal_object_ood_all_tasks_100ep_threshold_q95_20260610")
...
for task_id in range(18):
    ...
    cfg = {{
        ...
        # Conformal q95 threshold settings
        "selection_main_threshold": {Q95_THRESH},
        "selection_streak_threshold": {Q95_THRESH},
...
print("Generated 18 configs for modified_h10_risk_topk8 with threshold q95.")
...
def run():
    print("Starting threshold q95 sweep...", flush=True)
...
if p.returncode != 0:
    ...
    tmux_cmd = f"tmux new-session -d -s ood_production_threshold_q95_100ep_202610 'cd {NEW_ROOT} && python3 run_all.py > sweep_supervisor.log 2>&1'"
    ...
    if p_tmux.returncode != 0:
        ...
        print("TMUX session 'ood_production_threshold_q95_100ep_202610' launched successfully!")
