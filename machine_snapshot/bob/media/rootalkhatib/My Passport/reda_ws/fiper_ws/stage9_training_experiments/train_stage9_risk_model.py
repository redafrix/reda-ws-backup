...
from stage9_training_experiments.stage9_losses import compute_stage9_loss, risk_prob_from_outputs
from stage9_training_experiments.stage9_models import Stage9Dims, create_model, model_config


EVAL_SPLITS = ["calib", "test_seen_task", "test_unseen_group", "test_unseen_task", "test_unseen_seed", "test_unseen_perturbation"]
...
def main() -> None:
    ...
    parser.add_argument("--target-mode", required=True, choices=["clean_binary", "soft_labels", "soft_no_ambiguous", "ordinal", "subtype_multitask", "continuous_regression"])
    parser.add_argument("--source-remap", action="append")
    parser.add_argument("--epochs", type=int, default=8)
    parser.add_argument("--patience", type=int, default=15, help="Patience for early stopping, 0 to disable")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        calib_metrics = summarize_predictions(calib_rows, args.target_mode)["calib"]["clean_binary"]
        
        # Combined score to break ties when auroc_bad saturates at 1.0
        auroc = calib_metrics.get("auroc_bad")
        if auroc is None:
            auroc = 0.0
        brier = calib_metrics.get("brier")
        if brier is None:
            brier = 1.0
        score = (float(auroc), -float(brier))
        ...
        if score > best_score:
            ...
            if args.patience > 0 and epochs_no_improve >= args.patience:
                print(f"Early stopping triggered at epoch {epoch}. No improvement for {args.patience} epochs.", flush=True)
                break
    ...
    metrics = {
        ...
        "splits": summarize_predictions(all_predictions, args.target_mode),
    }
