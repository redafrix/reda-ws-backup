...
Updated: 2026-06-29 by Codex Bob status audit plus Pi0.5 official OOD update.
...
| `cross_suite_official_ood_20260630` | Bob + Sam | RUNNING as of 2026-06-30 13:58 CEST | Cross-suite official OOD offline campaign for H10 TopK8 vs official FIPER evidence. Sam official seen `libero_goal_object_official` H10 uncertainty collection was safely stopped at ~4,469 episodes and is transferring to Bob as the verified seen training source; 20/20 BDDL/init files match Hugging Face `zhouxueyang/LIBERO-Pro` by SHA256. Bob supervisor tmux `cross_suite_official_ood_20260630` is collecting official-file-validated OOD suites with modified SimVLA `ckpt-60000`, H10, 8 ACE candidates, 49D uncertainty, saved MuJoCo states, and max 300 timeout. Target datasets: `libero_goal_swap` 100ep, `libero_goal_task` 100ep, `libero_goal_object_ood` 180ep, `libero_spatial_object` 100ep, `libero_object_object` 100ep, `libero_10_object` 100ep. Relation/semantic/spatial-object/object-object/10-object OOD registry entries were excluded because Bob's official LIBERO-PRO tree has no matching BDDL/init files for them. Root: `/media/rootalkhatib/My Passport/reda_ws/fiper_ws/cross_suite_official_ood_20260630`. goal-object-OOD restored 2026-06-30 with documented local provenance (`libero_goal_object_ood_temp` BDDL + `libero_goal_object_ood` init). Supervisor log: `/media/rootalkhatib/My Passport/reda_ws/fiper_ws/cross_suite_official_ood_20260630/logs/supervisor.log`. |
...
| Item | Status |
| :--- | :--- |
| Running sessions | `cross_suite_official_ood_20260630`, `cross_suite_sam_seen_transfer_20260630` |
