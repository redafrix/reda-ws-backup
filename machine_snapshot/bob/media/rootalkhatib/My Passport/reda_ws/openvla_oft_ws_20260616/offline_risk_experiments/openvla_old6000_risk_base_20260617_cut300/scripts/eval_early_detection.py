...
def main():
        ...
        for line in f:
            ...
            if key in summaries_map:
                ...
                if global_id in test_ids:
                    # Truncate queries at 300 steps for failures
                    if not ep["success"] and q["env_timestep"] > 300:
                        continue
                    if ep["success"]:
    ...
    print(f"Test split: {len(test_failed_episodes)} failed episodes (capped at 300 steps), {len(test_success_episodes)} successful episodes.")
    ...
    print("\n=== Early Failure Detection Rates on Test Split (300 Steps Cutoff) ===")
    for r in early_ratios:
