...
# Paths
OUTPUT_DIR = "/media/rootalkhatib/My Passport/reda_ws/openvla_oft_ws_20260616/offline_risk_experiments/openvla_old6000_risk_base_20260617_cut300"
...
def build_features_and_targets():
    ...
    # We copy splits from the previous run folder to ensure EXACT same episode splits
    PREV_SPLITS_DIR = "/media/rootalkhatib/My Passport/reda_ws/openvla_oft_ws_20260616/offline_risk_experiments/openvla_old6000_risk_base_20260617/splits"
    with open(os.path.join(PREV_SPLITS_DIR, "train_episode_ids.json"), "r") as f:
        train_ids = set(json.load(f))
    with open(os.path.join(PREV_SPLITS_DIR, "val_episode_ids.json"), "r") as f:
        val_ids = set(json.load(f))
    with open(os.path.join(PREV_SPLITS_DIR, "test_episode_ids.json"), "r") as f:
        test_ids = set(json.load(f))

    # Save to the new split folder for reference
    ...
    for key, ep in episodes.items():
        ...
        history_buffer = []
        for q in queries:
            # Check cut-off at 300 steps for failures
            if not success and q["env_timestep"] > 300:
                continue
...
def main():
    ...
    print("\n--- Training SeqRiskModel Transformer (Truncated 300 steps) ---")
    model = SeqRiskModel(
    ...
    report_path = os.path.join(OUTPUT_DIR, "reports", "OPENVLA_OLD6000_OFFLINE_RISK_BASE_REPORT_300STEPS_20260617.md")
    os.makedirs(os.path.dirname(report_path), exist_ok=True)

    trans_val_metrics = results_val["best_val_f1"]
    trans_test_metrics = results_test["best_val_f1"]

    report_content = f"""# Offline Risk Model Evaluation Report (Truncated 300 Steps)
...
* **Train Split Queries:** {len(train_loader.dataset)}
* **Val Split Queries:** {len(val_loader.dataset)}
* **Test Split Queries:** {len(test_loader.dataset)}
