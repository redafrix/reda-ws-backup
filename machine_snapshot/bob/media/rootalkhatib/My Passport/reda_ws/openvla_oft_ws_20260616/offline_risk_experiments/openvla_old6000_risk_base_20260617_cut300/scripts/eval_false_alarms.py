...
def main():
    ...
    with open(queries_path, "r") as f:
        for line in f:
            ...
            if key in summaries_map:
                ...
                if global_id in test_ids and ep["success"]:
    ...
    print(f"Test split: {len(test_success_episodes)} successful episodes.")
    ...
    for gid, queries in test_success_episodes.items():
        ...
        for q in queries:
            ...
            episode_max_prob = max(episode_max_prob, prob)

            history_buffer.append((proprio, executed, ace))

        for k, th in thresholds.items():
            if episode_max_prob >= th:
                false_alarms[k] += 1

    print("\n=== Episode-level False Alarm Rate (FPR) on Successful Episodes ===")
    for k, count in false_alarms.items():
        rate = count / len(test_success_episodes) * 100
        print(f"  - Threshold {k} ({thresholds[k]:.4f}): {rate:.2f}% ({count}/{len(test_success_episodes)})")
