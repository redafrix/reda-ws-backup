#!/usr/bin/env python3
"""Closed-loop SimVLA pick-place rollout in the IsaacLab tabletop scene."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run closed-loop SimVLA pick-place rollouts.")
    parser.add_argument("--rollout_config", type=Path, default=Path("configs/eval_simvla_pick_place_paper_bob_5ep_no_rotation.yaml"))
...
def rollout_collection_config(rollout_cfg: dict) -> dict:
    ...
    for key in (
        ...
        "settle_time_s", "record_cameras", "record_depth", "camera_fps", "state_record_fps",
    ):
        if key in rollout_cfg:
            collection_cfg[key] = rollout_cfg[key]
    if int(collection_cfg.get("num_envs", 1)) != 1:
        raise ValueError("SimVLA pick-place rollout currently requires num_envs=1.")
...
def make_live_observation(scene, robot, ee_body_id: int, instruction: str, commanded_opening_m: float) -> SimVLALiveObservation:
...
def run_simvla_pick_place_episode(*, sim, scene, robot, ik, gripper, simvla_policy, collection_cfg: dict, episode_id: int, episode_plan, scene_assets, asset_bank, output_dir: Path, simulation_app, suite, seed: int) -> Path:
    ...
    print(f"[INFO] SimVLA pick-place episode {episode_id} success={success} completed={completed} steps={step} instruction={episode_plan.spec.instruction!r}", flush=True)
    if not success:
...
def run_rollouts(args: argparse.Namespace, simulation_app) -> None:
    ...
    print("[INFO] Loading SimVLA runtime for pick-place rollout...", flush=True)
    runtime.load()
    ...
    for episode_id in episode_ids:
        ...
        print(f"[INFO] Starting SimVLA pick-place episode {episode_id}: instruction={plan.spec.instruction!r}, target={assets.object_context.label!r}, placement={assets.placement_context.label!r}", flush=True)
        run_simvla_pick_place_episode(
            sim=sim, scene=scene, robot=robot, ik=ik, gripper=gripper, simvla_policy=simvla_policy,
