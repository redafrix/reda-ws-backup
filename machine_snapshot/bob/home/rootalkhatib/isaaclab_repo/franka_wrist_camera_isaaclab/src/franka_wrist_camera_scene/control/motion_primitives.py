...
def _require_finite_tensor(name: str, value: torch.Tensor) -> None:
    """Fail loudly when a trajectory calculation produces NaN or Inf."""
...
def _require_finite_float(name: str, value: float) -> None:
    """Fail loudly when a scalar trajectory calculation produces NaN or Inf."""
...
class MinimumJerkPoseMotion:
    ...
    def __post_init__(self) -> None:
        _require_finite_tensor("pose_motion.start_pos_w", self.start_pos_w)
        _require_finite_tensor("pose_motion.goal_pos_w", self.goal_pos_w)
        _require_finite_tensor("pose_motion.quat_w", self.quat_w)
    ...
    def from_speed(
        ...
        _require_finite_tensor("pose_motion.start_pos_w", start_pos_w)
        _require_finite_tensor("pose_motion.goal_pos_w", goal_pos_w)
        _require_finite_tensor("pose_motion.quat_w", quat_w)

        distance_m = float(torch.linalg.norm(goal_pos_w - start_pos_w, dim=-1).max().item())
        _require_finite_float("pose_motion.distance_m", distance_m)
        duration_s = max(
            MINIMUM_JERK_PEAK_VELOCITY_SCALE * distance_m / max_speed_m_s,
            MINIMUM_MOTION_DURATION_S,
        )
        _require_finite_float("pose_motion.duration_s", duration_s)
    ...
    def sample(self, sim_time_s: float) -> tuple[torch.Tensor, torch.Tensor, bool]:
        ...
        _require_finite_tensor("pose_motion.sample.pos_w", pos_w)
...
class MinimumJerkWaypointMotion:
    ...
    def from_segment_speeds(
        ...
        for index, waypoint in enumerate(waypoints_w):
            _require_finite_tensor(f"waypoint_motion.waypoints_w[{index}]", waypoint)
        _require_finite_tensor("waypoint_motion.quat_w", quat_w)
        ...
        _require_finite_tensor("waypoint_motion.segment_lengths", segment_lengths)
        speed_limits = torch.tensor(
            max_speed_m_s,
            device=segment_lengths.device,
            dtype=segment_lengths.dtype,
        )
        _require_finite_tensor("waypoint_motion.speed_limits", speed_limits)
        scaled_lengths = segment_lengths / speed_limits
        _require_finite_tensor("waypoint_motion.scaled_lengths", scaled_lengths)
        total_scaled_length = float(scaled_lengths.sum().item())
        _require_finite_float("waypoint_motion.total_scaled_length", total_scaled_length)
        duration_s = max(
            MINIMUM_JERK_PEAK_VELOCITY_SCALE * total_scaled_length,
            MINIMUM_MOTION_DURATION_S,
        )
        _require_finite_float("waypoint_motion.duration_s", duration_s)
        ...
        _require_finite_tensor("waypoint_motion.cumulative_scaled_lengths", cumulative_scaled_lengths)
    ...
    def sample(self, sim_time_s: float) -> tuple[torch.Tensor, torch.Tensor, bool]:
        ...
        _require_finite_tensor("waypoint_motion.total_scaled_length", total_scaled_length)
        target_scaled_length = alpha * total_scaled_length
        _require_finite_tensor("waypoint_motion.target_scaled_length", target_scaled_length)
        ...
        _require_finite_float("waypoint_motion.local_alpha", local_alpha)

        start = self.waypoints_w[segment_index]
        goal = self.waypoints_w[segment_index + 1]
        pos_w = start + local_alpha * (goal - start)
        _require_finite_tensor("waypoint_motion.sample.pos_w", pos_w)
        return pos_w, self.quat_w, done


@dataclass(slots=True)
class MinimumJerkPositionMotion:
    """Move between two Cartesian positions with minimum-jerk scalar progress."""
    ...
    def __post_init__(self) -> None:
        _require_finite_tensor("position_motion.start_pos_w", self.start_pos_w)
        _require_finite_tensor("position_motion.goal_pos_w", self.goal_pos_w)
        self.start_pos_w = self.start_pos_w.clone()
        self.goal_pos_w = self.goal_pos_w.clone()
    ...
    def from_speed(
        ...
        start_time_s: float,
        max_speed_m_s: float,
    ) -> "MinimumJerkPositionMotion":
        if max_speed_m_s <= 0.0:
            raise ValueError("max_speed_m_s must be positive.")
        _require_finite_tensor("position_motion.start_pos_w", start_pos_w)
        _require_finite_tensor("position_motion.goal_pos_w", goal_pos_w)

        distance_m = float(torch.linalg.norm(goal_pos_w - start_pos_w, dim=-1).max().item())
        _require_finite_float("position_motion.distance_m", distance_m)
        duration_s = max(
            MINIMUM_JERK_PEAK_VELOCITY_SCALE * distance_m / max_speed_m_s,
            MINIMUM_MOTION_DURATION_S,
        )
        _require_finite_float("position_motion.duration_s", duration_s)
    ...
    def sample(self, sim_time_s: float) -> tuple[torch.Tensor, bool]:
        """Sample target position at simulation time."""
        alpha, done = self.profile.sample(sim_time_s - self.start_time_s)
        pos_w = self.start_pos_w + alpha * (self.goal_pos_w - self.start_pos_w)
        _require_finite_tensor("position_motion.sample.pos_w", pos_w)
        return pos_w, done
