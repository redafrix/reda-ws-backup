...
SUPPORT_RECEPTACLE_CATEGORIES = {"plate", "tray"}
VISUAL_CONTAINER_CATEGORIES = {"cup"}
...
VERIFIED_PHYSICAL_CONTAINER_VARIANTS = {
    ("box", variant_id)
    for variant_id in HOLLOW_BOX_VARIANTS
}
...
VARIANT_AFFORDANCE_OVERRIDES = {
    variant_key: ["reachable", "container", "physical_container"]
    for variant_key in VERIFIED_PHYSICAL_CONTAINER_VARIANTS
}
VARIANT_GRASP_STRATEGY_OVERRIDES = {
    variant_key: "unsupported"
    for variant_key in VERIFIED_PHYSICAL_CONTAINER_VARIANTS
}
VARIANT_VISUAL_LABEL_OVERRIDES: dict[tuple[str, str], str] = {}
...
def affordances_for_label(label: str) -> list[str]:
    if label in SUPPORT_RECEPTACLE_CATEGORIES:
        return ["reachable", "support", "container", "physical_container"]

    if label in SUPPORT_CATEGORIES:
        return ["reachable", "support"]

    if label in VISUAL_CONTAINER_CATEGORIES:
        return ["reachable", "container"]

    if label in PHYSICAL_CONTAINER_CATEGORIES:
        return ["reachable", "container", "physical_container"]
...
def role_for_label(label: str) -> str:
    if label in SUPPORT_CATEGORIES and label not in SUPPORT_RECEPTACLE_CATEGORIES:
        return "clutter"
    return "target"
...
def grasp_strategy_for_label(label: str) -> str:
    if (
        label in SUPPORT_CATEGORIES
        or label in VISUAL_CONTAINER_CATEGORIES
        or label in PHYSICAL_CONTAINER_CATEGORIES
    ):
        return "unsupported"
...
def label_for_category_id(category_id: str) -> str:
    return CATEGORY_LABEL_OVERRIDES.get(category_id, category_id)
...
def collect_category_variants(asset_root: Path, category_dir: Path) -> list[dict]:
    ...
    for usd_path in sorted(category_dir.glob("*.usd")):
        ...
        visual_label = VARIANT_VISUAL_LABEL_OVERRIDES.get((category_dir.name, usd_path.stem))
        if visual_label is not None:
            variant["visual_label"] = visual_label
...
def generate_object_catalog(asset_root: Path) -> dict:
    ...
    for category_dir in sorted(path for path in asset_root.iterdir() if path.is_dir()):
        ...
        label = label_for_category_id(category_dir.name)
        categories.append(
            category_entry(
