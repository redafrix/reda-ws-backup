...
- Supervised risk models train on success_train_seen plus failure_train_seen
...
def make_vector_inputs(rows_by_split: dict[str, list[CompactRow]], cfg: dict[str, Any], mode: str) -> dict[str, Any]:
    if mode == "supervised":
        ...
        train_rows = rows_by_split["success_train_seen"]
        val_rows = rows_by_split["success_val_seen"]
        y_train = None
        y_val = None
...
def campaign_objective(metrics: dict[str, Any]) -> float:
    ...
    ood_fail = ep.get("failure_eval_ood", {}).get("or_q95_K3")
    ood_succ = ep.get("success_test_ood", {}).get("or_q95_K3")
    seen_succ = ep.get("success_test_seen", {}).get("or_q95_K3", {"episode_alarm_rate": 0.0})
    if ood_fail is None:
        # Global-main has no held-out OOD split. Use the clean seen-failure
        # test split for ranking, while keeping OOD fields null in summaries.
        ood_fail = ep.get("failure_test_seen", {}).get("or_q95_K3", {"det_at_25": 0.0, "episode_alarm_rate": 0.0})
    if ood_succ is None:
...
def run_one_job(
    ...
    # 7. Global validation before training
    ...
    if use_sequence:
        ...
        hist_dim = 0
        static_dim = inputs["x_train"].shape[-1]
    ...
    summary = {
        ...
        "key_metrics": {
            "success_test_seen_or_q95_K3_fa": metric_value("success_test_seen", "or_q95_K3", "episode_alarm_rate"),
            "success_test_ood_or_q95_K3_fa": metric_value("success_test_ood", "or_q95_K3", "episode_alarm_rate"),
            "failure_test_seen_or_q95_K3_det": metric_value("failure_test_seen", "or_q95_K3", "episode_alarm_rate"),
            "failure_test_seen_or_q95_K3_det25": metric_value("failure_test_seen", "or_q95_K3", "det_at_25"),
            "failure_eval_ood_or_q95_K3_det": metric_value("failure_eval_ood", "or_q95_K3", "episode_alarm_rate"),
            "failure_eval_ood_or_q95_K3_det25": metric_value("failure_eval_ood", "or_q95_K3", "det_at_25"),
            "failure_eval_ood_or_q95_K3_mean_time": metric_value("failure_eval_ood", "or_q95_K3", "mean_first_norm_detected"),
            "success_test_ood_score_q95_K3_fa": metric_value("success_test_ood", "score_q95_K3", "episode_alarm_rate"),
            "failure_eval_ood_score_q95_K3_det25": metric_value("failure_eval_ood", "score_q95_K3", "det_at_25"),
        },
    }
    ...
    with csv_path.open("w", newline="") as f:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "failure_test_seen_or_q95_K3_det",
                "failure_test_seen_or_q95_K3_det25",
            ],
        )
        writer.writeheader()
