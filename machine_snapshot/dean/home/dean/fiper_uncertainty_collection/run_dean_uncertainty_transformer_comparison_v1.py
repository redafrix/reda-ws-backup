...
FEATURE_VARIANTS = ["base", "uncertainty"]
...
def stratified_seen_buckets(
    ...
    def split_class(items: list[EpisodeMeta], train_frac: float):
        n = len(items)
    ...
    s_train, s_val, s_calib, s_test = split_class(succ, 0.55)
    f_train, f_val, _f_unused_calib, f_test = split_class(fail, 0.70)
    ...
    result = {
        ...
        "feature_audit": {
            ...
            "input_fields": [
                ...
                "current.proprio",
            ] + (["simvla_uncertainty_49d", "simvla_uncertainty_delta_49d"] if variant == "uncertainty" else []),
            "static_dim": 141 if variant == "uncertainty" else 43,
            "history_dim": 21,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir", default="/home/dean/fiper_uncertainty_collection/experiments/dean_uncertainty_transformer_comparison_v1_20260601")
    p.add_argument("--splits", nargs="+", default=["random_mixed", "ood_suite_libero90", "ood_task_holdout"])
