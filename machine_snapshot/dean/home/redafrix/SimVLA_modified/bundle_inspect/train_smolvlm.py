...
def get_args_parser():
    ...
    parser.add_argument("--ignore_mismatched_checkpoint_sizes", action="store_true", default=False,
                        help="Allow loading a base checkpoint with action heads whose shapes differ from the current action space")
    ...
    parser.add_argument("--predict_uncertainty", action="store_true", default=False,
                        help="Enable phase-1 uncertainty head and loss")
    parser.add_argument("--uncertainty_beta", type=float, default=0.5)
    ...
    parser.add_argument("--use_lora", action="store_true", default=False,
                        help="Use LoRA for VLM backbone")
...
def get_param_groups(model: SmolVLMVLA):
    ...
    if getattr(model, "use_lora", False):
        vlm_params = [p for n, p in model.vlm.named_parameters() if "lora_" in n]
    else:
        vlm_params = all_vlm_params

    if getattr(model.config, "predict_uncertainty", False):
        if hasattr(model.transformer, "velocity_final_layer"):
            action_params = (
                list(model.transformer.velocity_final_layer.parameters())
                + list(model.transformer.logvar_final_layer.parameters())
                + list(model.transformer.action_encoder.parameters())
            )
        else:
            action_params = (
                list(model.transformer.velocity_head.parameters())
                + list(model.transformer.logvar_head.parameters())
                + list(model.transformer.action_encoder.parameters())
            )
    elif hasattr(model.transformer, "final_layer"):
...
def log_trainable_params(model, logger):
    ...
    for n, p in model.named_parameters():
        ...
        if p.requires_grad:
            trainable += p.numel()
            logger.info(f"TRAINABLE: {n} | {p.numel()/1e6:.3f}M")
...
def set_trainable_groups(model: SmolVLMVLA, *, train_vlm: bool, train_transformer_core: bool, train_action_heads: bool):
    ...
    if use_lora:
        # In LoRA mode, we only toggle requires_grad on parameters that contain "lora_"
        # Base VLM parameters should remain frozen.
...
def update_group_lrs(optim, step, args):
    ...
    def get_lr(name, start_step):
        if not args.use_cosine_decay:
            return base[name]
        return linear_warmup_cosine(
            step, start_step, args.warmup_steps, 
            args.iters, base[name], args.min_lr_ratio
        )
    ...
    if args.use_lora and args.freeze_mode in ["freeze_vlm_entire_run", "freeze_vlm_only", "action_heads_only"]:
        # In LoRA mode, 'vlm' refers only to adapters, so it should be trainable
        if "vlm" not in trainable_during_freeze:
            trainable_during_freeze.append("vlm")
    ...
    if args.freeze_mode == "freeze_vlm_entire_run":
        ...
        for name in base:
            start_step = 0 if name in trainable_during_freeze else args.freeze_steps
            set_group_lr(optim, name, get_lr(name, start_step))
...
def main(args):
    ...
    if load_path and os.path.isdir(load_path) and os.path.exists(os.path.join(load_path, "model.safetensors")):
        ...
        # Manually load config and model to avoid 'meta device' RuntimeError in from_pretrained
        ...
        config.predict_uncertainty = args.predict_uncertainty
        config.uncertainty_beta = args.uncertainty_beta
        config.uncertainty_eps = args.uncertainty_eps
        ...
        state_dict = load_safetensors_file(os.path.join(load_path, "model.safetensors"), device="cpu")
        
        # Use the model's custom load_state_dict which handles uncertainty head upgrades
        model.load_state_dict(state_dict, strict=False)
        
        model._maybe_copy_legacy_output_head(str(load_path))
        ...
        if args.use_lora:
            logger.info("Enabling gradient checkpointing for VLM backbone (checkpoint path)")
    ...
    # Enable gradient checkpointing for VLM if LoRA is on to save VRAM
    if args.use_lora:
    ...
    if args.freeze_steps > 0:
        if args.freeze_mode == "action_heads_only":
            ...
            set_trainable_groups(
                accelerator.unwrap_model(model),
                train_vlm=args.use_lora,
                train_transformer_core=True,
                train_action_heads=True,
            )
            vlm_status = "training LoRA adapters" if args.use_lora else "frozen"
            logger.info(
                f"Real parameter freeze enabled for entire run: training transformer core + action heads, VLM {vlm_status}"
            )
            groups_unfrozen = True
            log_trainable_params(accelerator.unwrap_model(model), logger)
        ...
        groups_unfrozen = True
        logger.info("No freeze_steps requested: training all groups from step 0")
        log_trainable_params(accelerator.unwrap_model(model), logger)
    ...
    for batch in train_dataloader:
        ...
        if "loss" in loss_dict:
            loss = loss_dict["loss"]
        elif "velocity_nll_loss" in loss_dict:
            loss = loss_dict["velocity_nll_loss"]
        elif "velocity_loss" in loss_dict:
        ...
        if global_step % args.log_interval == 0:
            ...
            if accelerator.is_main_process:
                ...
                extra_logs = ""
                if "mean_var" in logs:
                    extra_logs += f" var={logs['mean_var']:.4f}"
                if "mse_proxy" in logs:
                    extra_logs += f" mse={logs['mse_proxy']:.4f}"
