...
def _variance_row(uncertainty: Dict, key: str, plan_step_idx: int) -> Optional[List[float]]:
    ...
    if arr.ndim >= 2:
        if arr.shape[0] == 0:
            return None
        idx = max(0, min(int(plan_step_idx), arr.shape[0] - 1))
        arr = arr[idx]
    return np.ravel(arr).astype(float).tolist()
# Constants
# -----------------------------------------------------------------------------
...
class WebSocketClient:
    ...
    def reset(self) -> None:
        ...
        self.plan_info: Deque[Dict] = collections.deque()
        self.last_step_info: Optional[Dict] = None
    ...
    def step(self, obs: Dict, goal: str) -> np.ndarray:
        if not self.action_plan:
            ...
            path_step_mean = uncertainty.get("path_step_mean", [])
            last_step_mean = uncertainty.get("last_step_mean", [])
            path_variance = uncertainty.get("path_variance")
            last_step_variance = uncertainty.get("last_step_variance")
            ...
            for i in range(min(self.replan_steps, len(action_chunk))):
                step_info = {
                    "plan_step_idx": i,
                    "path_step_mean": float(path_step_mean[i]) if i < len(path_step_mean) else None,
                    "last_step_mean": float(last_step_mean[i]) if i < len(last_step_mean) else None,
                    "path_variance": _variance_row(uncertainty, "path_variance", i) if path_variance is not None else None,
                    "last_step_variance": _variance_row(uncertainty, "last_step_variance", i) if last_step_variance is not None else None,
                }
                for key in PLAN_SCALAR_UNCERTAINTY_KEYS:
                    step_info[key] = uncertainty.get(key)
                self.action_plan.append(action_chunk[i])
                self.plan_info.append(step_info)

        action = self.action_plan.popleft()
        self.last_step_info = self.plan_info.popleft() if self.plan_info else None
        return action
...
class HTTPClient:
    ...
    def infer(self, element: Dict) -> Dict:
        try:
            ...
            if "uncertainty" in data:
                result["uncertainty"] = data["uncertainty"]
            return result
    ...
    def step(self, obs: Dict, goal: str) -> np.ndarray:
        if not self.action_plan:
            ...
            for i, action in enumerate(action_chunk[:self.replan_steps]):
                step_info = {
                    "plan_step_idx": i,
                    "path_step_mean": float(path_step_mean[i]) if i < len(path_step_mean) else None,
                    "last_step_mean": float(last_step_mean[i]) if i < len(last_step_mean) else None,
...
def eval_libero(
    ...
    uncertainty_log_path: Optional[str] = None,
) -> float:
    """
    ...
    if task_suite_name not in benchmark_dict:
        raise ValueError(
            f"Unknown task suite '{task_suite_name}'. Available suites: {sorted(benchmark_dict.keys())}"
        )
    ...
    uncertainty_log_file = None
    if uncertainty_log_path:
        Path(uncertainty_log_path).parent.mkdir(parents=True, exist_ok=True)
        uncertainty_log_file = open(uncertainty_log_path, "a", encoding="utf-8")
    ...
    for task_id in tqdm(range(num_tasks - 1, -1, -1), desc="Tasks"):
        ...
        try:
            ...
            print(f"   [SKIP] Task {task_id}: failed to load init states ({e})")
            continue
        if initial_states is None or len(initial_states) == 0:
            print(f"   [SKIP] Task {task_id}: no init states available")
            continue
        ...
        for ep in tqdm(range(num_trials), desc=f"{task_description[:30]}...", leave=False):
            ...
            replay_images = []
            uncertainty_trace: List[Dict[str, float]] = []
            t = 0
            done = False
            ...
            while t < max_steps + NUM_STEPS_WAIT:
                try:
                    ...
                    step_info = getattr(client, "last_step_info", None)
                    if step_info is not None:
                        uncertainty_trace.append({
                            "env_step": int(t),
                            "plan_step_idx": int(step_info.get("plan_step_idx", -1)),
                            "path_step_mean": step_info.get("path_step_mean"),
                            "last_step_mean": step_info.get("last_step_mean"),
                            "path_variance": step_info.get("path_variance"),
                            "last_step_variance": step_info.get("last_step_variance"),
                            **{key: step_info.get(key) for key in PLAN_SCALAR_UNCERTAINTY_KEYS},
                        })
            ...
            if uncertainty_log_file is not None:
                path_vals = [x["path_step_mean"] for x in uncertainty_trace if x["path_step_mean"] is not None]
                last_vals = [x["last_step_mean"] for x in uncertainty_trace if x["last_step_mean"] is not None]
                record = {
                    ...
                    "success": bool(done),
                    "steps": int(t),
                    "mean_path_uncertainty": float(np.mean(path_vals)) if path_vals else None,
                    "max_path_uncertainty": float(np.max(path_vals)) if path_vals else None,
                    "mean_last_step_uncertainty": float(np.mean(last_vals)) if last_vals else None,
                    "max_last_step_uncertainty": float(np.max(last_vals)) if last_vals else None,
                    "uncertainty_trace": uncertainty_trace,
                }
                uncertainty_log_file.write(json.dumps(record) + "\n")
                uncertainty_log_file.flush()

        env.close()
        task_rate = task_successes / num_trials if num_trials > 0 else 0.0
        print(f"   Task {task_id}: {task_successes}/{num_trials} ({task_rate*100:.1f}%)")
    
    success_rate = total_successes / max(total_episodes, 1)
    print(f"\nTotal success rate: {total_successes}/{total_episodes} ({success_rate*100:.1f}%)")
    
    if uncertainty_log_file is not None:
        uncertainty_log_file.close()
...
def main():
    ...
    parser.add_argument("--task_suite", type=str, default="libero_spatial",
                        help="Benchmark suite name, e.g. libero_10 or a LIBERO-PRO suite like libero_10_temp")
    ...
    parser.add_argument("--task_id", type=int, default=None,
                        help="If set, evaluate only this task id within the suite")
    parser.add_argument("--uncertainty_log", type=str, default=None,
                        help="Optional JSONL path for per-episode uncertainty logging")
    ...
    eval_libero(
        ...
        uncertainty_log_path=args.uncertainty_log,
    )
