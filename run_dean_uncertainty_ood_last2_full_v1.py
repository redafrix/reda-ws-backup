...
BAD_TASKS = {("libero_goal_object", 9), ("libero_10_object", 4)}
WORKER_DIRS = ["worker_0", "worker_1", "worker_2", "worker_3"]
FEATURE_VARIANTS = ["base", "unc_raw", "unc_summary", "unc_summary_only", "unc_raw_dropout"]
LAST2_TASK_OOD = {
    "libero_10_object": {8, 9},
    "libero_90": {88, 89},
    "libero_goal_object": {7, 8},
    "libero_object_object": {8, 9},
    "libero_spatial_object": {8, 9},
}
...
def load_episode_meta(run_root: Path) -> dict[str, EpisodeMeta]:
    ...
    for worker_dir in WORKER_DIRS:
        ...
        for row in read_jsonl(path):
            ...
            episodes[eid] = EpisodeMeta(
                ...
                sweep_idx=int(row.get("sweep_idx") or 0),
            )
...
def split_bool_ood(name: str, ep: EpisodeMeta) -> bool:
    if name in {"random_mixed", "all_tasks_random"}:
        return False
    if name == "ood_last2_taskids_full":
        return ep.task_id in LAST2_TASK_OOD.get(ep.suite, set())
...
def stratified_seen_buckets(
    ...
    s_train, s_val, s_calib, s_test = split_class(succ, 0.55, 0.15, 0.15)
    f_train, f_val, _f_unused_calib, f_test = split_class(fail, 0.55, 0.15, 0.0)
...
def stratified_seen_buckets_no_test(
    seen: list[EpisodeMeta],
    rng: random.Random,
    limits: dict[str, int],
) -> dict[str, set[str]]:
    ...
    n_s_train = int(len(succ) * 0.70)
    n_s_val = int(len(succ) * 0.15)
    s_train = succ[:n_s_train]
    s_val = succ[n_s_train : n_s_train + n_s_val]
    s_calib = succ[n_s_train + n_s_val :]

    n_f_train = int(len(fail) * 0.80)
    f_train = fail[:n_f_train]
    f_val = fail[n_f_train:]
    ...
    return {
        ...
        "failure_val_seen": {e.episode_id for e in take(f_val, limits["val_failure"])},
        "success_calib_seen": {e.episode_id for e in take(s_calib, limits["calib_success"])},
        "success_test_seen": set(),
        "failure_test_seen": set(),
    }
...
def make_split_assignments(
    ...
    if split_name == "ood_last2_taskids_full":
        buckets = stratified_seen_buckets_no_test(seen, rng, limits)
    else:
        buckets = stratified_seen_buckets(seen, rng, limits)
...
def build_rows_for_split(
    ...
    for worker_dir in WORKER_DIRS:
        ...
        for raw in read_jsonl(path):
            ...
            executed = pad_flat(raw.get("executed_action"), 7)
            ...
            used_rows += 1
    print(f"Built rows: parsed={parsed_rows}, used={used_rows}", flush=True)
...
def make_arrays(rows: list[RowEx], variant: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, list[str], np.ndarray]:
    ...
    if variant in {"uncertainty", "unc_raw", "unc_raw_dropout"}:
...
def summarize_available(episodes: dict[str, EpisodeMeta]) -> dict[str, Any]:
    ...
    return {
        ...
        "excluded_bad_tasks": sorted([f"{s}/task_{t}" for s, t in BAD_TASKS]),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir", default="/home/dean/fiper_uncertainty_collection/experiments/dean_ood_last2_taskids_full_v1_20260601")
    p.add_argument("--splits", nargs="+", default=["ood_last2_taskids_full"])
    ...
    p.add_argument("--train-success-limit", type=int, default=260)
    p.add_argument("--train-failure-limit", type=int, default=260)
    p.add_argument("--val-success-limit", type=int, default=80)
    p.add_argument("--val-failure-limit", type=int, default=80)
    p.add_argument("--calib-success-limit", type=int, default=120)
    p.add_argument("--test-success-limit", type=int, default=180)
    p.add_argument("--test-failure-limit", type=int, default=180)
    p.add_argument("--ood-success-limit", type=int, default=260)
    p.add_argument("--ood-failure-limit", type=int, default=260)
    return p.parse_args()
...
def main() -> None:
    ...
    episodes = load_episode_meta(run_root)
    ...
    for split_name in args.splits:
        ...
        rows_by_bucket = build_rows_for_split(run_root, episodes, buckets, args.history_steps)
