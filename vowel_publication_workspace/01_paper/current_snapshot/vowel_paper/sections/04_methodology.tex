...
The VLA monitor uses a late-fusion Transformer. Its sequence path combines a
16-query execution history with the current \(10\times7\) proposed action.
Each history token contains proprioception, the previous executed action and
six candidate-disagreement statistics. A static path receives 28 action
statistics, seven candidate-set summaries, current proprioception and the
eight selected uncertainty features. Both paths have width 128; a three-layer,
four-head Transformer encodes the sequence before fusion with the static
embedding.
...
cause the eventual failure. Writing \(F_e=1-Y_e\) for episode failure, their
classification objective has the form
\begin{equation}
  \begin{aligned}
    W_b&=\sum_{e,t}w_{e,t}^b,\\
    \mathcal{L}_{\mathrm{risk}}^b
    &=-\frac{1}{W_b}\sum_{e,t} w_{e,t}^b
    \left[
      F_e\log s_{e,t}^b \right.\\
    &\hspace{28mm}\left.
      +(1-F_e)\log(1-s_{e,t}^b)
    \right].
  \end{aligned}
  \label{eq:risk-loss}
\end{equation}
where \(w_{e,t}^b\) accounts for the sampling and class balancing used by each
training set. Consequently, \(s_t^b\) is an empirical warning score for the
observed episode prefix, not a causal probability that executing \(A_t^b\)
will fail.

Each score is compared only with its branch threshold. The VLA uses the stored
95th-percentile operating point from its successful calibration split. The
world-model threshold is calibrated from the episode maximum score on
...
distribution shift. The selected controller fixes
\(\tau_\vla=0.6155\) and uses \(\alpha=0.20\), which gives
\(\tau_\wm=0.6968\).
