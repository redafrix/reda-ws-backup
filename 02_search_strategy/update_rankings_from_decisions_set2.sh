...
final_set="$vault/05 Reviews/Final Must-Read Set 2.md"
reading_queue="$vault/05 Reviews/Reading Queue Set 2.md"
ranked_current="$vault/05 Reviews/Ranked Final Current Set 2.md"
ranked_source="$vault/05 Reviews/Ranked Final Current Set 2.md"
pdf_summaries="$vault/05 Reviews/PDF-Backed Read-First Summaries Set 2.md"
...
for f in "$paper_dir"/*.md; do
  if rg -q 'graph/final-must-read-set-2' "$f"; then
    name="$(basename "$f" .md)"
    rank="$(sed -n 's/^project_rank_set2: //p' "$f" | head -n1)"
    desired_rank="$(sed -n 's/^desired_rank_set2: //p' "$f" | head -n1)"
    decision="$(sed -n 's/^reader_decision_set2: //p' "$f" | head -n1)"
    if [[ -z "$decision" ]]; then
      decision="keep"
    fi
...
for line in "${ordered_lines[@]}"; do
  ...
  rank=$((rank + 1))
  perl -0pi -e "s/^project_rank_set2: \\d+/project_rank_set2: $rank/m" "$path"
  perl -0pi -e "s/maxValue\\(\\d+\\)/maxValue\\($rank\\)/" "$path"
...
pattern = r"(## Final Must-Read Papers\n\nPapers are ranked here from most important to least important for this second set\.\n\n)(.*?)(\n## Fastest Way To Read This Set)"
...
path, block = sys.argv[1], sys.argv[2]
text = open(path, encoding="utf-8").read()
pattern = r"(## Read First: Set 2\n\n)(.*?)(\nUse \[\[05 Reviews/Ranked Final Current Set 2\]\] if you only want the ranked order with one-line reasons\.)"
text = re.sub(pattern, lambda m: m.group(1) + block.rstrip() + "\n" + m.group(3), text, flags=re.S)
open(path, "w", encoding="utf-8").write(text)
PY
...
---
title: Ranked Final Current Set 2
type: review
tags:
  - internship/review
  - internship/ranking
...
# Ranked Final Current Set 2

This ranking is rebuilt from each paper's \`reader_decision_set2\` property.
...
echo "Updated rankings for $rank selected papers in set 2."
