...
> Updated: 2026-06-23 by Codex full workspace audit plus Sam H10 OOD correction.
...
| OpenVLA final goal-object dataset | **TRUST dataset** (1,890 complete `libero_goal_object` episodes; final OpenVLA risk training dataset) |
| OpenVLA final1890 offline risk model | **TRUST offline** (current OpenVLA goal-object risk model; different feature schema from SimVLA/FIPER) |
| OpenVLA old-goal to goal-object OOD transfer tests | **TRUST diagnostic** (strict offline cross-dataset tests; old plain-goal model partially transfers but is not the final goal-object model) |
| OpenVLA online OOD basic-vs-risk run | **TRUST positive, modest** (complete 3,600/3,600; basic H8 976/1,800 = 54.22%, adaptive risk H1/8 1,014/1,800 = 56.33%, net +38 successes) |
| SimVLA plain-goal to goal-object offline OOD | **TRUST offline diagnostic, negative transfer** (source heldout AUROC 0.9307/AUPRC 0.9051; full goal-object OOD AUROC 0.7627/AUPRC 0.6998; best-val-F1 OOD false alarm 4.59%, failure detection 78.91%, but Det@25 0.15% and Det@50 7.61%; worse than previous best offline detector on failure detection, Det@25, Det@50, and mean detection time) |
| Dean official-FIPER materialized-code run 20260622 | **TRUST offline (Verified 2026-06-24)** (complete/frozen: both Option A and Option B runs succeeded; best row is `rnd_oe_and_entropy Option B (OOD-only)` with Success FA 38.9%, Failure Det 97.6%, Accuracy 79.4%, Det. Time 0.314, Det@10 11.9%, Det@25 52.4%, Det@50 78.6%) |
| Sam 18-task OOD 180ep H10 collection 20260622 | **TRUST dataset + detector-only diagnostic** (complete/frozen: 180 episodes, 10/task across official `libero_goal_object_ood`, H10, 8 ACE candidates, 49D uncertainty; corrected H10 TopK8 detector audit uses online ACE formula and shows saved threshold under-calibrated on official OOD: saved `q95_mass_0.15` FA 96.0% / Det 100%, while `q95_mass_20` gives FA 20.8% / Det 96.8% / Det@25 90.3% / Det@50 96.8%) |
| Sam 18-task OOD 180ep H10 cap-300 derivative 20260623 | **TRUST derived detector-only diagnostic** (derived from the official OOD 180ep dataset by keeping `timestep < 300`; 180 episodes, 143 successes, 37 failures; six max-800 successes became cap-300 failures; `q95_mass_20` gives FA 18.9% / Det 91.9% / Det@25 0.0% / Det@50 83.8%; use for horizon sensitivity only, not as fresh recollection) |
| Model identity verification | **TRUST** (Step 7: 0 mismatches) |
| Suite identity verification | **TRUST** (Step 8: 0 fallbacks) |
| Pi0.5 frozen goal-object dataset | **TRUST dataset** (4,090 complete round-robin `libero_goal_object` episodes; final Pi0.5 risk training dataset) |
| Pi0.5 goal-object H10 offline risk model | **TRUST offline** (complete/frozen training on Bob; best Val-F1 threshold 0.4800, q95 0.7218; test AUROC 0.9534, AUPRC 0.9728; test step FPR 10.53%, step FNR 10.88%; episode False Alarm at q95_mass_10 is 2.98% with 99.21% Failure Detection and Det@50 92.1%; highly effective offline risk head) |
| Pi0.5 official `libero_goal_swap` 50ep online/offline audit | **TRUST complete, weak positive online** (Bob; 500 basic + 500 risk selected-cap episodes, max 300, H10. Online basic 161/500 = 32.20%; risk selected-cap 166/500 = 33.20%, net +5. Risk made 1,306 action changes, mostly on failed episodes: 1,017/1,306. Offline old-with-task9 threshold sweep shows no strong operating point on `goal_swap`: `q95_mass_1` gives about 31-33% FA and 83-85% failure detection; `q95_mass_1.25` lowers FA to about 20% but failure detection is about 81%.) |
| Pi0.5 official `libero_goal_object_ood` 18-task 100ep online/offline audit | **TRUST complete, net negative online** (Bob; 1,800 basic + 1,800 risk selected-cap episodes, max 300, H10. Online basic 1,754/1,800 = 97.44%; risk selected-cap 1,736/1,800 = 96.44%, net -18. Risk made 759 action changes across 529 episodes; changes are not selective enough on failures for this easy official OOD suite.) |
| Official FIPER OOD180 threshold sweep | **TRUST diagnostic sweep, not deployment** (Dean; no retrain, same 180 OOD episodes, reused official RND checkpoints. Official deployment point `any_1` still gives 100% Success FA / 100% Failure Det. OOD operating-curve sweep exposes lower-FA diagnostic rows, e.g. `rnd_oe_and_entropy mass_above_1_50` = 6.0% FA / 100.0% Det / Det@50 96.8%, but this uses OOD-test-set threshold selection and is not the unchanged FIPER method.) |
| Official FIPER OOD180 cap-300 threshold sweep | **TRUST diagnostic sweep, not deployment** (Dean; no retrain, same 180 OOD episodes relabeled with cap-300 rule: 143 success / 37 failure. Official `any_1` still gives 100% Success FA / 100% Failure Det. Low-FA mass thresholds degrade sharply: `rnd_oe_and_entropy mass_above_1_50` gives 2.8% FA / 91.9% Det / Det@25 0.0% / Det@50 59.5%, showing late/missed detection under the cap-300 criterion.) |
| H10 TopK8 OOD180 cap-300 extended threshold sweep | **TRUST diagnostic sweep, not deployment** (Dean; no retrain and same cap-300 143 success / 37 failure labels. Best balanced sweep row found: `fixed_0.5_mass_30` = 17.5% Success FA / 91.9% Failure Det / Det@50 83.8%. Low-FA row `q99_mass_2` = 1.4% Success FA / 83.8% Failure Det / Det@50 27.0%, showing late detection.) |
| H10 TopK8 seen-only recalibration on OOD180 | **TRUST negative calibration audit** (Bob+Dean, 2026-06-29; no retrain. Thresholds selected only from seen `libero_goal_object` buckets: `success_calib_seen`, `success_val_seen`, and optionally `failure_val_seen`; then applied once to official OOD180. Result remains poor: best strict actual-max800 row shown in report is `q99_seen_success_FA010` with 57.7% Success FA / 100.0% Failure Det / Det@25 90.3%; cap-300 gives 57.3% FA / 89.2% Det. Conclusion: seen-only calibration does not fix the OOD false-alarm shift.) |
...
| SimVLA online cross-suite argmin-cap replay | **TRUST positive but modest online deployment evidence** (Bob, complete 2026-07-09. Paired replay on the exact official cross-suite episodes from `cross_suite_official_ood_20260630`, using same BDDLs, init states, seeds, timestep-0 MuJoCo states, and max-300 timeout. Baseline original SimVLA H10: 392/680 = 57.65%. Risk-aware modified SimVLA H10 + promoted seen-only TopK8 risk head + argmin-cap: 403/680 = 59.26%, net +11 successes. Thresholds were fixed per suite before replay: `goal_object_ood_180` best-val-F1/q90-success, `goal_task_100` best-val-F1/q90-success, the other four suites q95-success/q99-success. Report: `/media/rootalkhatib/My Passport/reda_ws/fiper_ws/cross_suite_official_ood_20260630/experiments/online_cross_suite_argmin_caps_20260709/ONLINE_CROSS_SUITE_ARGMIN_CAPS_SUMMARY.md`.) |
