...
## 18. Franka Reaching pi0.5 Full-Pose v3 Dataset (Dean, 2026-07-23)

| Property | Value |
| :--- | :--- |
| **Purpose** | Canonical stock-pi0.5 fine-tuning dataset for Isaac Lab Franka reaching. |
| **Raw Source** | `/media/redafrix/My Passport/reaching_pose_v1_4400` on the external disk now attached to Dean. |
| **Raw Audit** | 4,000 train + 400 validation episodes; `train/002237` excluded because `meta.json` and `rgb.npz` were corrupt. |
| **Converted Dataset** | `/mnt/ai/pi05/datasets/reaching_pose_v1_4400_pi05_fullpose_v3` |
| **Train Split** | 3,999 episodes, 720,976 frames. |
| **ID Validation Split** | 400 episodes, 71,069 frames; excluded from training and normalization. |
| **Inputs** | Natural-language instruction, external RGB, wrist RGB, 15D Franka state. |
| **Actions** | Stored full-pose 8D record; 7D semantic policy action; horizon 10; stock OpenPI padding to 32D. |
| **V3 Change** | State-only rewrite of EE orientation columns `state[3:6]`; images and stored actions unchanged. |
| **Normalization** | Fresh train-only statistics under `norm_stats/pi05_franka_reach_fullpose_v3_stock/train/norm_stats.json`. |
| **Validation** | Finite values, exact unchanged columns/actions/timestamps/splits, deterministic image equality, OpenPI loader smoke, orientation round-trip validation. |
| **Training Use** | `/mnt/ai/pi05/training/reaching_pose_v1_4400_pi05_fullpose_v3`, config `pi05_franka_reach_fullpose_v3_stock_production`. |
| **Status** | Canonical current dataset. The earlier v2 custom-loss checkpoints are archived and must not be resumed. |
