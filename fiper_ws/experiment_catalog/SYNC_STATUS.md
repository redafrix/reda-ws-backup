...
Last updated: 2026-07-01 (promoted cross-suite official OOD model).
...
## 2026-07-03 Cross-Machine Catalog Consolidation

- Created the new primary entrypoint: `CROSS_MACHINE_EXPERIMENT_MAP_20260703.md`.
- Created the large-artifact Git substitute manifest: `BIG_ARTIFACTS_NOT_IN_GIT_20260703.md`.
- Created `GIT_SYNC_PLAN_20260703.md` documenting the current Git blocker and the branch-per-host push plan.
- Audited local/Bob/Sam/Dean state before writing:
  - Bob (`pcrobot`): reachable, `/media/rootalkhatib/My Passport` has about 101G free but is 95% used; current key roots are `fiper_ws`, `cross_suite_official_ood_20260630`, `official_fiper_original_bob_20260701`, `openvla_oft_ws_20260616`, `pi05_libero_risk_ws_20260623`, and the separate Isaac Lab workspace `/home/rootalkhatib/isaaclab_repo/franka_wrist_camera_isaaclab`.
  - Sam: reachable, root disk critically full at about 1.9G free; official source dataset `simvla_official_libero_goal_object_h10_uncertainty_17410ep_20260626` exists with 4,469 episodes and 1,060,884 sample rows.
  - Dean: reachable, root disk critically full at about 3.8G free; official FIPER/H10 OOD evaluation roots remain present.
  - Local/Batman: Obsidian report path verified; local `.git` is an empty/broken directory, so Git push is blocked until the intended GitHub remote/repo is restored or provided.
- Verified Bob official FIPER reports exist:
  - `/media/rootalkhatib/My Passport/reda_ws/fiper_ws/official_fiper_original_bob_20260701/official_fiper_seen_train_eval_20260701/OFFICIAL_FIPER_SEEN_TRAIN_EVAL_REPORT.md`
  - `/media/rootalkhatib/My Passport/reda_ws/fiper_ws/official_fiper_original_bob_20260701/official_fiper_seen_thresholds_cross_suite_ood_20260702/OFFICIAL_FIPER_SEEN_THRESHOLDS_CROSS_SUITE_OOD_20260702.md`
- Verified Bob cross-suite OOD datasets:
  - source seen: 4,469 episodes, 52.25GB JSONL sample file.
  - OOD targets: `goal_swap_100`, `goal_task_100`, `goal_object_ood_180`, `spatial_object_100`, `object_object_100`, `libero10_object_100`.
- Next sync step: copy updated catalog files to Bob, Sam, and Dean.

## 2026-07-01 Promoted Cross-Suite Model Sync

- Local catalog updated with promoted model `simvla_h10_topk8_official_goal_object_seen_main_20260701`.
- Bob catalog sync: PASS.
- Sam catalog sync: PASS.
- Dean catalog sync: BLOCKED because `/dev/sda2` is full (`No space left on device`). Do not treat Dean catalog as current until disk space is freed and the sync is retried.
...
## 2026-07-10 Online Cross-Suite Argmin-Cap Result Added

- Added Bob online deployment result `online_cross_suite_argmin_caps_20260709` to:
  - `MASTER_EXPERIMENT_INDEX.md`;
  - `KEY_RESULTS.md`;
  - `TRUSTED_RESULTS_SUMMARY.md`;
  - `CROSS_MACHINE_EXPERIMENT_MAP_20260703.md`.
- Result summary:
  - original SimVLA H10: 392/680 = 57.65%;
  - risk-aware modified SimVLA H10 + promoted TopK8 argmin-cap: 403/680 = 59.26%;
  - net gain: +11 paired online successes;
  - total risk action changes: 2,529 across 449 changed risk episodes.
- Source report:
  - `/media/rootalkhatib/My Passport/reda_ws/fiper_ws/cross_suite_official_ood_20260630/experiments/online_cross_suite_argmin_caps_20260709/ONLINE_CROSS_SUITE_ARGMIN_CAPS_SUMMARY.md`.
- Protocol note:
  - same official cross-suite BDDLs, init states, reset seeds, timestep-0 MuJoCo states, and max-300 timeout as the offline datasets;
  - thresholds fixed per suite before online replay;
  - no OOD retraining.

## 2026-07-30 Dean Hardware Replacement, SSD Relocation, And Isaac/pi0.5 Context

- Verified direct SSH alias `dean` now reaches the replacement workstation:
  hostname `Batman`, SSH user `dean`, RTX 5090 32,607 MiB, NVIDIA driver
  `580.95.05`.
- Verified the former Bob 2 TB `My Passport` SSD experiment disk is attached to
  Dean at `/media/redafrix/My Passport1` (`/dev/sda1`, exFAT,
  non-rotational).
- Verified Dean's `/media/redafrix/My Passport` is a separate 4 TB HDD
  (`/dev/sdb1`, NTFS, rotational), not the disk moved from Bob.
- Verified Bob remains reachable as `pcrobot`, but
  `/media/rootalkhatib/My Passport/reda_ws` is absent there.
- Recorded the dedicated Dean AI SSD `/mnt/ai` and the current active roots:
  `/mnt/ai/pi05`, `/mnt/ai/isaac`, and
  `/mnt/ai/projects/SimVLA_Gontary`.
- Read the complete Dean Codex session
  `019f7eed-ad2e-7893-88dd-17d805ce9187` (109 turn contexts, 14,294 JSONL
  events) and cross-checked its principal reports and live processes.
- Current pi0.5 v3 stock training is active. Live evidence reached optimizer
  step 24,500/30,000 with finite stock 32D loss/gradients, effective batch 256,
  RTX 5090 at full utilization, and finalized checkpoints through 24,000.
- Registered the archived noncanonical v2 custom-loss run, canonical v3 dataset
  and trainer, SimVLA RTX 5090 setup, and exact OOD150 protocol discovery.
- The supplied SimVLA OOD150 counts 77/150 at 2 cm and 88/150 at 4 cm are not
  yet marked trusted because the exact checkpoint hash and generated owner
  result payload are absent.
- Updated local catalog files. The current synchronization targets are:
  ...
  - Bob fallback: `/home/rootalkhatib/experiment_catalog_current`.
  Do not attempt the old Bob external-disk destination while the disk is on
  Dean.
