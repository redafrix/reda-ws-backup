...
Updated: 2026-07-30 after the Dean workstation replacement, SSD relocation, and
Isaac/pi0.5 campaign audit.
...
## Current Infrastructure Transition (2026-07-30)

The 2 TB external `My Passport` SSD experiment disk was physically moved from Bob
to the replacement Dean workstation.

| Item | Current state |
|---|---|
| Historical Bob experiment tree | Now physically available on Dean at `/media/redafrix/My Passport1/reda_ws`; absent from Bob's old mount path |
| Bob | Reachable as `pcrobot`, but old `/media/rootalkhatib/My Passport/reda_ws` paths are not mounted |
| Replacement Dean | Direct SSH alias `dean`; hostname `Batman`; SSH user `dean`; workloads run as `redafrix` |
| Dean GPU | NVIDIA GeForce RTX 5090, 32,607 MiB, driver `580.95.05` |
| Dean AI SSD | `/mnt/ai`, internal Samsung 9100 PRO 1 TB, ext4 label `AI_SSD` |
| Dean external disks | `/media/redafrix/My Passport1` (2 TB SSD, former Bob disk, `/dev/sda1`, exFAT, non-rotational) and `/media/redafrix/My Passport` (separate 4 TB HDD, `/dev/sdb1`, NTFS, rotational) |
| Access caveat | Desktop-user external mounts may reject noninteractive reads from SSH user `dean`; switch to the `redafrix` workload context when needed |
...
| Bob / PCROBOTUBUNTU02 | `pcrobot` | Historical online/eval compute host | External `My Passport` is no longer attached | Do not run paths under `/media/rootalkhatib/My Passport` unless the volume is deliberately returned and verified. |
...
| Dean / Batman replacement workstation | `dean` | Current Isaac Lab, OpenPI/pi0.5 training, SimVLA reaching evaluation; also hosts relocated historical Bob disk | `/`: 74G free; `/mnt/ai`: 374G free at audit | Use `/mnt/ai` for active AI work. Query `redafrix` processes/logs rather than assuming `dean` user-service state. |
...
| Online cross-suite argmin-cap policy | Bob: `/media/rootalkhatib/My Passport/reda_ws/fiper_ws/cross_suite_official_ood_20260630/experiments/online_cross_suite_argmin_caps_20260709` | Same official cross-suite replay episodes as `cross_suite_official_ood_20260630` | original SimVLA H10 baseline vs modified SimVLA `ckpt-60000` + promoted TopK8 risk head; corrected ACE `new_training` | Current best cross-suite online SimVLA deployment evidence | Trusted positive but modest: 392/680 baseline vs 403/680 risk-aware, net +11; suite-dependent, small regression on `libero10_object_100`. |
...
| pi0.5 Franka v3 stock fine-tune | Dean: `/mnt/ai/pi05/training/reaching_pose_v1_4400_pi05_fullpose_v3` | 3,999 train episodes / 720,976 frames; 400 held-out ID validation episodes / 71,069 frames | language, external RGB, wrist RGB, 15D Franka state, horizon-10 7D semantic action padded to stock 32D | Current Isaac reaching policy training | Canonical replacement for the archived v2 custom-loss run. Uses stock pi0.5 32D loss, effective batch 256, target 30k updates. Active at step 24,500 on 2026-07-30. |
...
| Franka reaching raw dataset | Dean external disk: `/media/redafrix/My Passport/reaching_pose_v1_4400` | 4,000 train + 400 validation episodes; one corrupt train episode excluded downstream | Source Isaac reaching data for pi0.5 conversion | Preserved and read-only during conversion. |
| Franka reaching pi0.5 v3 dataset | Dean AI SSD: `/mnt/ai/pi05/datasets/reaching_pose_v1_4400_pi05_fullpose_v3` | 3,999 train / 720,976 frames; 400 ID val / 71,069 frames | Current stock pi0.5 fine-tuning dataset | Validated state-only v3 rewrite with fresh train-only normalization. |

## Isaac and pi0.5 Transition (2026-07-20 to 2026-07-30)

The detailed Dean Codex history is session
`019f7eed-ad2e-7893-88dd-17d805ce9187`.

| Campaign / artifact | Status | Evidence |
|---|---|---|
| OpenPI/pi0.5 workstation setup | Complete | `/mnt/ai/pi05/setup/PI05_SETUP_REPORT.md` |
| Isaac Sim/Lab + Franka collection/deployment integration | Complete smoke pipeline | `/mnt/ai/pi05/franka_reaching/PI05_END_TO_END_SMOKE_REPORT.md` |
| Full-pose v2 custom-loss training | Archived, noncanonical, do not resume | `/mnt/ai/pi05/audits/pi05_fullpose_v2_adversarial_audit/PI05_FULLPOSE_ADVERSARIAL_AUDIT.md` |
| Full-pose v3 stock training | Active | `/mnt/ai/pi05/training/reaching_pose_v1_4400_pi05_fullpose_v3/reports/PI05_FRANKA_V3_STOCK_FINETUNE_LAUNCH_REPORT.md` |
| Latest live training audit | 24,500/30,000, finite stock 32D loss and gradients; checkpoint 24,000 finalized | production log under the v3 training root |
| SimVLA RTX 5090 setup | Complete except deferred live model-load while pi0.5 owns the GPU | `/mnt/ai/projects/SimVLA_Gontary/WORKSTATION_SETUP.md` |
| SimVLA Isaac OOD150 protocol discovery | Protocol found; exact owner result/checkpoint not yet present | `/mnt/ai/isaac/SIMVLA_OOD150_EVALUATION_DISCOVERY_REPORT.md` |

The OOD150 protocol contains 150 deterministic full-OOD scenes. The primary
metric is 2 cm success sustained for 0.2 seconds; 4 cm is a counterfactual
secondary metric computed from the same trajectories. The supplied `77/150`
and `88/150` counts are not yet independently certified because the exact
checkpoint hash and generated result directory are absent.
...
| Online cross-suite argmin-cap replay | `/media/rootalkhatib/My Passport/reda_ws/fiper_ws/cross_suite_official_ood_20260630/experiments/online_cross_suite_argmin_caps_20260709/ONLINE_CROSS_SUITE_ARGMIN_CAPS_SUMMARY.md` |
...
6. Before any Bob-path operation, remember that the external experiment disk is
   currently on Dean and Bob's historical mount path is absent.
7. For current Isaac/pi0.5 work, start with the 2026-07-30 transition section
   and the Dean Codex session/report paths above.
