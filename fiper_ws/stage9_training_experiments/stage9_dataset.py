...
def target_values(label: str, target_mode: str, bad_subtype: str, sample: dict[str, Any] | None = None) -> tuple[float, float, int, int]:
    if target_mode == "continuous_regression":
        ...
        if sample is not None:
            ...
            if isinstance(cr, dict):
                risk_score = cr.get("risk_score", 0.0)
                risk_confidence = cr.get("risk_confidence", 1.0)
            else:
                lbl = sample.get("label")
                if isinstance(lbl, dict):
                    risk_score = lbl.get("risk_score", 0.0)
                    risk_confidence = lbl.get("risk_confidence", 1.0)
                else:
                    risk_score = sample.get("risk_score", 0.0)
                    risk_confidence = sample.get("risk_confidence", 1.0)
        lbl_str = label
        if isinstance(lbl_str, dict):
            lbl_str = lbl_str.get("final_label") or lbl_str.get("label")
        ord_val = LABEL_TO_ORDINAL.get(lbl_str, -1)
        sub_id = SUBTYPE_TO_ID.get(bad_subtype, -1)
        return float(risk_score) if risk_score is not None else 0.0, float(risk_confidence) if risk_confidence is not None else 1.0, ord_val, sub_id
...
class Stage9RiskDataset(Dataset):
    def __init__(
        ...
        self.rows = [r for r in self.rows if target_values(r["label"], target_mode, r["bad_subtype"], r)[1] > 0]
        if balance_binary:
    ...
    def _load_samples(self, split_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for row in split_rows:
            source_str = str(row.get("source_jsonl", ""))
            if not source_str:
                continue
            source = resolve_path(source_str, self.source_remaps)
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        y, weight, ordinal, subtype_id = target_values(label, self.target_mode, subtype, sample)
