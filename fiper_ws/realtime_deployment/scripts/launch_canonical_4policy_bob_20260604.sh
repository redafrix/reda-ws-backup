...
record_status() {
  ...
  if [[ -n "$code" ]]; then
    printf '{"time":"%s","host":"bob","stage":"%s","event":"%s","code":%s}\n' "$now" "$stage" "$event" "$code" >> "$STATUS"
  else
    printf '{"time":"%s","host":"bob","stage":"%s","event":"%s"}\n' "$now" "$stage" "$event" >> "$STATUS"
  fi
}
...
run_stage() {
  ...
  python3 "$RUNNER" --config "$config" --policy "$policy" --episode-start 0 --episode-end 100 > "$log" 2>&1
  local code=$?
...
run_stage "01_original_simvla" "simvla_only" "$ROOT/realtime_deployment/configs/${RUN_ID}_bob_original_simvla.json"
run_stage "02_modified_simvla_ckpt60000" "simvla_only" "$ROOT/realtime_deployment/configs/${RUN_ID}_bob_modified_simvla_ckpt60000.json"
run_stage "03_risk_base" "risk_base" "$ROOT/realtime_deployment/configs/${RUN_ID}_bob_risk_base.json"
run_stage "04_risk_unc_topk8" "risk_unc_topk8" "$ROOT/realtime_deployment/configs/${RUN_ID}_bob_risk_unc_topk8.json"
