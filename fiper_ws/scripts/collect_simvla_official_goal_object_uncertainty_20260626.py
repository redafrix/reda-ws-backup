...
OFFICIAL_GOAL_OBJECT_SUITE = "libero_goal_object_official"
OFFICIAL_GOAL_OBJECT_TASKS = [
...
class ManualOfficialGoalObjectBenchmark:
    def __init__(self) -> None:
        self.tasks = [
            ManualOfficialTask(
                problem_folder="libero_goal_object_official",
                bddl_file=f"{stem}.bddl",
                init_states_file=f"{stem}.pruned_init",
                language=language,
            )
            for stem, language in OFFICIAL_GOAL_OBJECT_TASKS
        ]
        self.n_tasks = len(self.tasks)

    def get_task(self, task_id: int) -> ManualOfficialTask:
        return self.tasks[int(task_id)]


def add_manual_official_goal_object_benchmark(benchmark_dict: dict[str, Any]) -> dict[str, Any]:
    if OFFICIAL_GOAL_OBJECT_SUITE not in benchmark_dict:
        benchmark_dict[OFFICIAL_GOAL_OBJECT_SUITE] = ManualOfficialGoalObjectBenchmark
...
def make_env(benchmark_dict: dict[str, Any], get_libero_path_fn: Any, offscreen_cls: Any, suite: str, task_id: int, resolution: int, env_seed: int):
    ...
    if task.problem_folder == "libero_goal_object_official":
        folder_candidates.append("libero_goal_object_official")
...
def collect(args: argparse.Namespace) -> None:
    ...
    benchmark_dict = add_manual_official_goal_object_benchmark(benchmark.get_benchmark_dict())
